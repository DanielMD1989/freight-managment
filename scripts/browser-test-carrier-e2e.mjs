  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    if (status === 429 || (data?.error && data.error.includes('Too many'))) {
      if (attempt < 3) {
        ...
        console.log(`   ⏳ Rate limited (attempt ${attempt}/3). Waiting ${waitSec}s...`);
...
async function browserLogin(page, email, password) {
  ...
  if (!url.includes('/login')) {
    ...
    console.log(`   ✓ Saved cookies for ${email} (${cookieCache[email].length} cookies)`);
  }
...
async function phase1(browser, state) {
  ...
  try {
    ...
    // Select carrier type
    await page.selectOption('#carrierType, select[name="carrierType"]', 'CARRIER_COMPANY');
    await sleep(500);

    // Company name
    await page.fill('#companyName, input[name="companyName"]', CARRIER.companyName);

    // Password fields
    await page.fill('#password, input[name="password"]', CARRIER.password);
    await page.fill('#confirmPassword, input[name="confirmPassword"]', CARRIER.password);
    ...
    record('Phase 1', '1a. Register carrier (browser)', registrationSucceeded ? 'PASS' : 'FAIL', afterRegUrl);
    await screenshot(page, 'e2e-03-register-after');

    // Capture error if present
    if (!registrationSucceeded) {
      const pageText = await safeText(page);
      const errorMatch = pageText.match(/(User with.*?exists|Too many|Validation|Password)/i);
  ...
  try {
    ...
    loginOk = true;
    record('Phase 1', '1b. Login (fresh registration)', 'PASS',
  ...
  if (!loginOk) {
    ...
    try {
      ...
      if (regRes.ok) {
        ...
        console.log(`   ⚠ API registration failed: ${regRes.data?.error || regRes.status}`);
      }
  ...
  if (!loginOk) {
    console.log(`   ⚠ Registration rate-limited. Using seeded carrier: ${FALLBACK_CARRIER.email}`);
    try {
      const login = await apiLogin(FALLBACK_CARRIER.email, FALLBACK_CARRIER.password);
      state.carrier = {
        ...FALLBACK_CARRIER,
        token: login.sessionToken,
        userId: login.user.id,
      ...
      loginOk = true;
      record('Phase 1', '1b. Login (seeded carrier fallback)', 'PASS',
...
async function phase2(state) {
  ...
  if (state.usedFallbackCarrier) {
    record('Phase 2', '2. SKIP — using seeded carrier (docs exist)', 'PASS', '');
    return;
  }
  ...
  try {
    const { csrfToken } = await apiLoginWithCSRF(apiCtx, state.carrier.email, state.carrier.password);

    const docs = [
      { type: 'COMPANY_LICENSE', name: 'company-license.pdf', mime: 'application/pdf', buf: pdfBuffer },
      { type: 'TIN_CERTIFICATE', name: 'tin-cert.jpg', mime: 'image/jpeg', buf: jpegBuffer },
      { type: 'INSURANCE_CERTIFICATE', name: 'insurance.pdf', mime: 'application/pdf', buf: pdfBuffer },
    ];
    ...
    for (const doc of docs) {
      ...
      const ok = res.status() === 200 && data.document?.verificationStatus === 'PENDING';
      record('Phase 2', `2. Upload ${doc.type}`, ok ? 'PASS' : 'FAIL',
...
async function phase3(browser, state) {
  ...
  if (state.usedFallbackCarrier) {
    ...
    try {
      ...
      const balRes = await apiCall('GET', '/api/wallet/balance', { token: state.shipper.token });
      const wallets = balRes.data?.wallets || [];
      const shipperWallet = wallets.find(w => w.type === 'SHIPPER_WALLET' || w.accountType === 'SHIPPER_WALLET');
      const shipperBalance = Number(shipperWallet?.balance ?? 0);
      console.log(`   Shipper balance: ${shipperBalance} ETB`);

      if (shipperBalance < 50000) {
        const shipperSearchRes = await apiCall('GET', `/api/admin/users?search=${encodeURIComponent(SHIPPER.email)}`, {
          token: state.admin.token,
        });
        const shipperUser = shipperSearchRes.data?.users?.[0];
        if (shipperUser) {
          await apiCall('POST', `/api/admin/users/${shipperUser.id}/wallet/topup`, {
            body: { amount: 50000, paymentMethod: 'MANUAL', notes: 'E2E test shipper wallet top-up' },
            token: state.admin.token,
          });
        }
      }
    ...
    try {
      const balRes = await apiCall('GET', '/api/wallet/balance', { token: state.carrier.token });
      const wallets = balRes.data?.wallets || [];
      const carrierWallet = wallets.find(w => w.type === 'CARRIER_WALLET' || w.accountType === 'CARRIER_WALLET');
      const carrierBalance = Number(carrierWallet?.balance ?? 0);
      console.log(`   Carrier balance: ${carrierBalance} ETB`);

      if (carrierBalance < 50000) {
        const topUpRes = await apiCall('POST', `/api/admin/users/${state.carrier.userId}/wallet/topup`, {
          body: { amount: 50000, paymentMethod: 'MANUAL', notes: 'E2E test carrier wallet top-up' },
          token: state.admin.token,
        });
  ...
  try {
    ...
    await page.goto(`${BASE_URL}/admin/users/${state.carrier.userId}`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);
    ...
    const activateRes = await apiCall('PATCH', `/api/admin/users/${state.carrier.userId}`, {
      body: { status: 'ACTIVE' },
      token: state.admin.token,
    });
    record('Phase 3', '3a. Activate carrier user', activateRes.ok ? 'PASS' : 'FAIL',
  ...
  try {
    const verifyOrgRes = await apiCall('POST', `/api/admin/organizations/${state.carrier.orgId}/verify`, {
      token: state.admin.token,
    });
    record('Phase 3', '3b. Verify carrier org', verifyOrgRes.ok ? 'PASS' : 'FAIL',
  ...
  try {
    ...
    record('Phase 3', '3c. Top up carrier wallet (50,000 ETB)', topUpRes.ok ? 'PASS' : 'FAIL',
  ...
  try {
    ...
    if (shipperBalance < 50000) {
      ...
      const shipperUser = shipperSearchRes.data?.users?.[0];
      if (shipperUser) {
        const topUpRes = await apiCall('POST', `/api/admin/users/${shipperUser.id}/wallet/topup`, {
          body: { amount: 50000, paymentMethod: 'MANUAL', notes: 'E2E test shipper wallet top-up' },
          token: state.admin.token,
        });
        record('Phase 3', '3d. Top up shipper wallet', topUpRes.ok ? 'PASS' : 'FAIL',
          `status=${topUpRes.status}`);
      }
    } else {
      record('Phase 3', '3d. Shipper wallet sufficient', 'PASS', `balance=${shipperBalance}`);
    }
  } catch (e) {
  ...
  try {
    ...
    const carrierDocs = pendingDocs.filter(d =>
      d.organizationId === state.carrier.orgId || d.organization?.id === state.carrier.orgId
    );
    ...
    record('Phase 3', '3e. Approve carrier documents',
      approvedCount === carrierDocs.length && carrierDocs.length > 0 ? 'PASS' : 'FAIL',
  ...
  try {
    ...
    record('Phase 3', '3f. Carrier full access', fullAccess ? 'PASS' : 'FAIL',
...
async function phase4(browser, state) {
  ...
  try {
    ...
    await page.goto(`${BASE_URL}/carrier/trucks/add`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);
    ...
    const truckSubmitted = afterUrl.includes('/carrier/trucks') && !afterUrl.includes('/add');
    record('Phase 4', '4a. Submit truck form', truckSubmitted ? 'PASS' : 'FAIL', afterUrl);
  ...
  try {
    ...
    const trucksRes = await apiCall('GET', '/api/trucks?approvalStatus=PENDING', {
    ...
    if (ourTruck) {
      ...
      const allTrucksRes = await apiCall('GET', '/api/trucks', { token: state.carrier.token });
      ...
      if (fallback) {
        ...
        if (createdTruck) {
          ...
          record('Phase 4', '4a. Truck created via API fallback', 'PASS',
            `truckId=${state.truckId?.slice(-8)}`);
        } else {
          console.log(`   ⚠ Truck creation failed: ${JSON.stringify(createRes.data?.error || createRes.data)}`);
          record('Phase 4', '4a. Truck creation failed', 'FAIL',
...
async function phase5(browser, state) {
  ...
  try {
    ...
    await page.goto(`${BASE_URL}/admin/trucks/pending`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);
    ...
    const approveRes = await apiCall('POST', `/api/trucks/${state.truckId}/approve`, {
      body: { action: 'APPROVE' },
      token: state.admin.token,
    });
    if (approveRes.ok) {
    ...
    if (!approved && truckFound) {
      ...
      const approveClicked = await tryClick(page, [
      ...
      if (approveClicked) {
        ...
        const confirmClicked = await tryClick(page, [
    ...
    record('Phase 5', '5. Admin approves truck', approved ? 'PASS' : 'FAIL',
      `truckId=${state.truckId?.slice(-8)}`);
...
async function phase6(browser, state) {
  ...
  try {
    // Login as carrier
    await browserLogin(page, state.carrier?.email || CARRIER.email, state.carrier?.password || CARRIER.password);

    // Navigate to loadboard
    await page.goto(`${BASE_URL}/carrier/loadboard`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);
    ...
    if (postClicked) {
      ...
      try {
        ...
        const truckDropdown = await page.$('select:has(option:text("Select a truck"))');
        if (truckDropdown) {
          // Try selecting by truck ID
          if (state.truckId) {
            await truckDropdown.selectOption(state.truckId);
            truckSelected = true;
            console.log(`   ✓ Selected truck by ID: ${state.truckId?.slice(-8)}`);
          }
        }
        ...
        if (!truckSelected) {
          ...
          for (const sel of selects) {
            ...
            if (options.length > 1) {
              ...
              if (firstOptionText.includes('Select a truck') || firstOptionText.includes('Select')) {
                // Select the first real option (index 1)
                const secondOption = await options[1]?.getAttribute('value');
                if (secondOption) {
                  await sel.selectOption(secondOption);
                  truckSelected = true;
                  console.log(`   ✓ Selected first available truck: ${secondOption.slice(-8)}`);
                  break;
                }
              }
            }
        ...
        console.log(`   ⚠ Could not select truck: ${e.message?.split('\n')[0]}`);
      }
      ...
      if (truckSelected) {
        ...
        const originInput = await page.$('input[placeholder="Where is truck available?"]');
        if (originInput) {
          await originInput.fill('Addis Ababa');
          await sleep(1000);
          // Try to pick suggestion from dropdown
          const suggestion = await page.$('.pac-item, [role="option"]:has-text("Addis"), li:has-text("Addis")');
          if (suggestion) {
        ...
        console.log('   ⚠ No truck available in dropdown — will use API fallback');
    ...
    const postings = postingsRes.data?.truckPostings || postingsRes.data?.postings || [];
    const ourPosting = postings.find(p =>
      (p.truckId === state.truckId) || (p.truck?.id === state.truckId)
    );
    ...
    if (!ourPosting) {
      ...
      const citiesRes = await apiCall('GET', '/api/ethiopian-locations', { token: state.carrier.token });
      const cities = citiesRes.data?.locations || citiesRes.data?.cities || [];
      const addisAbaba = cities.find(c => c.name === 'Addis Ababa');
      console.log(`   Found ${cities.length} locations, Addis Ababa: ${addisAbaba?.id?.slice(-8) || 'NOT FOUND'}`);
      ...
      if (addisAbaba && state.truckId) {
        ...
        if (!postingRes.ok) {
          console.log(`   API fallback error: ${JSON.stringify(postingRes.data?.error || postingRes.data)}`);
        }
        record('Phase 6', '6. Post truck (API fallback)', postingRes.ok ? 'PASS' : 'FAIL',
          `status=${postingRes.status}`);
        if (postingRes.ok) {
          state.postingId = postingRes.data?.id;
        }
      } else {
        record('Phase 6', '6. Post truck (API fallback)', 'FAIL',
          `addisAbaba=${!!addisAbaba}, truckId=${!!state.truckId}`);
      }
    } else {
      record('Phase 6', '6. Post truck on loadboard', 'PASS',
...
async function phase7(browser, state) {
  ...
  try {
    ...
    state.loadId = loadId;
    record('Phase 7', '7a. Shipper creates load (API)', loadRes.status === 201 ? 'PASS' : 'FAIL',
  ...
  try {
    ...
    const loadVisible = loadboardText.includes('Dire Dawa') || loadboardText.includes('Steel') || loadboardText.includes('Addis Ababa') || loadboardText.includes('FLATBED');
    record('Phase 7', '7b. Load visible on carrier loadboard', loadVisible ? 'PASS' : 'FAIL',
  ...
  try {
    if (!state.loadId || !state.truckId) {
      throw new Error(`Missing loadId=${state.loadId} or truckId=${state.truckId}`);
    }
    ...
    state.loadRequestId = reqRes.data?.loadRequest?.id || reqRes.data?.request?.id;
    if (!reqRes.ok) {
      console.log(`   Load request error: ${JSON.stringify(reqRes.data?.error || reqRes.data)}`);
    }
    record('Phase 7', '7c. Carrier requests load (API)', reqRes.status === 201 ? 'PASS' : 'FAIL',
  ...
  try {
    ...
    const approveRes = await apiCall('POST', `/api/load-requests/${state.loadRequestId}/respond`, {
      body: { action: 'APPROVE', responseNotes: 'Approved - proceed to pickup' },
      token: state.shipper.token,
    });
    ...
    record('Phase 7', '7d. Shipper approves → trip created', approveRes.ok && tripId ? 'PASS' : 'FAIL',
...
async function phase8(browser, state) {
  ...
  try {
    const corridorsRes = await apiCall('GET', '/api/admin/corridors?originRegion=Addis+Ababa&destinationRegion=Dire+Dawa', {
      token: state.admin.token,
    });
    const corridors = corridorsRes.data?.corridors || [];
    const corridor = corridors.find(c =>
      c.originRegion?.includes('Addis') && c.destinationRegion?.includes('Dire')
    );
    if (corridor) {
      const shipperRate = Number(corridor.shipperPricePerKm || corridor.pricePerKm || 0);
      const carrierRate = Number(corridor.carrierPricePerKm || corridor.pricePerKm || 0);
      ...
      console.log(`   Shipper rate: ${shipperRate} ETB/km → expected fee: ${EXPECTED.shipperFee}`);
      console.log(`   Carrier rate: ${carrierRate} ETB/km → expected fee: ${EXPECTED.carrierFee}`);
  ...
  try {
    await browserLogin(page, state.carrier?.email || CARRIER.email, state.carrier?.password || CARRIER.password);
    await page.goto(`${BASE_URL}/carrier/trips`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);

    const tripsText = await safeText(page);
    const tripVisible = tripsText.includes('Dire Dawa') || tripsText.includes('Addis Ababa') || tripsText.includes('ASSIGNED');
    record('Phase 8', '8a. Trip visible on carrier trips page', tripVisible ? 'PASS' : 'FAIL', '');
  ...
  try {
    const carrierBal = await apiCall('GET', '/api/wallet/balance', { token: state.carrier.token });
    const shipperBal = await apiCall('GET', '/api/wallet/balance', { token: state.shipper.token });
    ...
    for (const w of carrierWallets) {
      console.log(`     - ${w.type || w.accountType}: ${w.balance} (id=${w.id?.slice(-8)})`);
    }
    ...
    const cw = carrierWallets.find(w => w.type === 'CARRIER_WALLET' || w.accountType === 'CARRIER_WALLET');
    const sw = shipperWallets.find(w => w.type === 'SHIPPER_WALLET' || w.accountType === 'SHIPPER_WALLET');
    ...
    record('Phase 8', '8b. Wallet balances recorded', 'PASS',
  ...
  const transitions = [
    ...
    { status: 'DELIVERED', label: 'IN_TRANSIT → DELIVERED', extra: { receiverName: 'Dawit G.', receiverPhone: '+251933333333' } },
  ];
  ...
  for (const t of transitions) {
    try {
      ...
      record('Phase 8', `8c. ${t.label}`, res.ok ? 'PASS' : 'FAIL',
        `status=${res.status}`);
    } catch (e) {
  ...
  try {
    await page.goto(`${BASE_URL}/carrier/trips/${state.tripId}`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);
  ...
  try {
    ...
    try {
      ...
      const podRes = await apiCtx.post(`${BASE_URL}/api/trips/${state.tripId}/pod`, {
        multipart: {
          file: { name: 'pod-delivery.jpg', mimeType: 'image/jpeg', buffer: podBuffer },
      ...
      record('Phase 8', '8d-i. Upload POD (carrier)', podOk ? 'PASS' : 'FAIL',
        `status=${podRes.status()}`);

      // If trip pod endpoint failed, try loads/[id]/pod
      if (!podOk) {
        const loadPodRes = await apiCtx.post(`${BASE_URL}/api/loads/${state.loadId}/pod`, {
          multipart: {
            file: { name: 'pod-delivery.jpg', mimeType: 'image/jpeg', buffer: podBuffer },
          },
          headers: { 'X-CSRF-Token': csrfToken },
        });
        record('Phase 8', '8d-i. Upload POD (fallback loads endpoint)',
  ...
  try {
    ...
    if (feeRes.ok && serviceFee) {
      console.log(`   Fee deduction: shipperFee=${serviceFee?.shipperFee}, carrierFee=${serviceFee?.carrierFee}, total=${serviceFee?.totalPlatformFee}`);
      record('Phase 8', '8e. Load → COMPLETED + fees deducted', 'PASS',
        `shipperFee=${serviceFee?.shipperFee}, carrierFee=${serviceFee?.carrierFee}, total=${serviceFee?.totalPlatformFee}`);
    } else if (feeRes.ok) {
      record('Phase 8', '8e. Load → COMPLETED (no fee data)', 'PASS',
        `status=${feeRes.status}`);
    } else {
      console.log(`   Status endpoint error: ${JSON.stringify(feeRes.data)}`);
      record('Phase 8', '8e. Load → COMPLETED', 'FAIL',
...
async function phase9(browser, state) {
  ...
  try {
    ...
    try {
      const carrierRelogin = await apiLogin(state.carrier.email, state.carrier.password);
    ...
    console.log(`   Carrier wallet response: status=${carrierBal.status}, wallets=${JSON.stringify(carrierBal.data?.wallets?.map(w => ({ type: w.type, balance: w.balance })))}`);
    console.log(`   Shipper wallet response: status=${shipperBal.status}, wallets=${JSON.stringify(shipperBal.data?.wallets?.map(w => ({ type: w.type, balance: w.balance })))}`);
  ...
  console.log('\n[9b] Fee verification');
  const carrierDeduction = state.carrierBalanceBefore - state.carrierBalanceAfter;
  const shipperDeduction = state.shipperBalanceBefore - state.shipperBalanceAfter;

  console.log(`   Carrier deduction: ${carrierDeduction} ETB (expected: ${EXPECTED.carrierFee})`);
  console.log(`   Shipper deduction: ${shipperDeduction} ETB (expected: ${EXPECTED.shipperFee})`);
  console.log(`   Total platform:   ${carrierDeduction + shipperDeduction} ETB (expected: ${EXPECTED.totalPlatformFee})`);

  // Allow 1% tolerance for rounding
  const carrierFeeOk = Math.abs(carrierDeduction - EXPECTED.carrierFee) <= EXPECTED.carrierFee * 0.01;
  const shipperFeeOk = Math.abs(shipperDeduction - EXPECTED.shipperFee) <= EXPECTED.shipperFee * 0.01;
  const totalOk = Math.abs((carrierDeduction + shipperDeduction) - EXPECTED.totalPlatformFee) <= EXPECTED.totalPlatformFee * 0.01;

  record('Phase 9', `9b-i. Carrier fee ≈ ${EXPECTED.carrierFee} ETB`, carrierFeeOk ? 'PASS' : 'FAIL',
    `actual=${carrierDeduction}, expected=${EXPECTED.carrierFee}`);
  record('Phase 9', `9b-ii. Shipper fee ≈ ${EXPECTED.shipperFee} ETB`, shipperFeeOk ? 'PASS' : 'FAIL',
    `actual=${shipperDeduction}, expected=${EXPECTED.shipperFee}`);
  record('Phase 9', `9b-iii. Total platform ≈ ${EXPECTED.totalPlatformFee} ETB`, totalOk ? 'PASS' : 'FAIL',
  ...
  try {
    const txRes = await apiCall('GET', '/api/wallet/transactions', { token: state.carrier.token });
    const transactions = txRes.data?.transactions || txRes.data?.recentTransactions || [];
    const feeTransaction = transactions.find(t =>
      t.type === 'SERVICE_FEE_DEDUCT' || t.description?.includes('Service fee') || t.description?.includes('service fee')
    );
    record('Phase 9', '9c. SERVICE_FEE_DEDUCT transaction exists',
  ...
  try {
    await browserLogin(page, state.carrier?.email || CARRIER.email, state.carrier?.password || CARRIER.password);
    await page.goto(`${BASE_URL}/carrier/wallet`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);

    const walletText = await safeText(page);
    const hasBalance = walletText.includes('Balance') || walletText.includes('ETB');
    const hasFeeEntry = walletText.includes('Service') || walletText.includes('fee') || walletText.includes('Deduct');
    record('Phase 9', '9d. Carrier wallet page shows balance', hasBalance ? 'PASS' : 'FAIL',
  ...
  try {
    ...
    await page.goto(`${BASE_URL}/shipper/wallet`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);
...
function phase10(state) {
  ...
  for (const r of results) {
    const status = r.status === 'PASS' ? '✓ PASS'
      : r.status === 'FAIL' ? '✗ FAIL'
      : '⚠ WARN';
    console.log(`| ${r.phase.padEnd(9)} | ${r.test.padEnd(48)} | ${status.padEnd(6)} |`);
  }
  ...
  // Fee breakdown
  const carrierDeduction = (state.carrierBalanceBefore || 0) - (state.carrierBalanceAfter || 0);
  const shipperDeduction = (state.shipperBalanceBefore || 0) - (state.shipperBalanceAfter || 0);

  console.log('\n' + '─'.repeat(60));
  console.log(`  FEE BREAKDOWN — Addis Ababa → Dire Dawa (${EXPECTED.distanceKm} km)`);
  console.log('─'.repeat(60));
  console.log(`  | Party    | Rate/km  | Expected    | Actual      | Match  |`);
  console.log(`  |----------|----------|-------------|-------------|--------|`);
  console.log(`  | Shipper  | ${String(EXPECTED.shipperRatePerKm + ' ETB').padEnd(8)} | ${String(EXPECTED.shipperFee).padEnd(11)} | ${String(shipperDeduction.toFixed(2)).padEnd(11)} | ${Math.abs(shipperDeduction - EXPECTED.shipperFee) <= 1 ? '✓' : '✗'}      |`);
  console.log(`  | Carrier  | ${String(EXPECTED.carrierRatePerKm + ' ETB').padEnd(8)} | ${String(EXPECTED.carrierFee).padEnd(11)} | ${String(carrierDeduction.toFixed(2)).padEnd(11)} | ${Math.abs(carrierDeduction - EXPECTED.carrierFee) <= 1 ? '✓' : '✗'}      |`);
  console.log(`  | Total    |          | ${String(EXPECTED.totalPlatformFee).padEnd(11)} | ${String((shipperDeduction + carrierDeduction).toFixed(2)).padEnd(11)} | ${Math.abs((shipperDeduction + carrierDeduction) - EXPECTED.totalPlatformFee) <= 2 ? '✓' : '✗'}      |`);
...
async function main() {
  ...
  console.log('║  E2E Carrier Workflow Test — Complete Lifecycle                    ║');
