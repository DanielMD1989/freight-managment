...
async function testWebCarrier(browser) {
  ...
  try {
    ...
    await page.fill('input[name="password"], input[type="password"]', CARRIER_PASSWORD);
    ...
    // ── 2. DASHBOARD ──────────────────────────────────────────────────
    console.log('\n[WEB] 2. Dashboard');
    await page.goto(`${WEB_URL}/carrier/dashboard`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);

    const dashText = await safeText(page);
    r.dashboardStats = hasText(dashText, 'Total Trucks', 'Available Trucks', 'Trucks on Job', 'Pending Approvals', 'Wallet Balance');
    r.dashboardQuickActions = hasText(dashText, 'Post Truck', 'Search Loads', 'Register Truck');
    r.dashboardSections = hasText(dashText, 'My Active Jobs', 'Available Loads', 'Fleet Overview');
    ...
    // ── 3. LOADBOARD ──────────────────────────────────────────────────
    console.log('\n[WEB] 3. Loadboard');
    await page.goto(`${WEB_URL}/carrier/loadboard`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);

    const lbText = await safeText(page);
    r.loadboardPage = hasText(lbText, 'My Trucks', 'Load Board', 'Loadboard', 'Post');
    ...
    const searchLoadsClicked = await tryClick(page, [
    ...
    if (searchLoadsClicked) {
      ...
      r.loadboardSearchLoads = hasText(searchText, 'Search', 'Load', 'Origin', 'Destination', 'Filter');
    ...
    // ── 4. TRUCKS ─────────────────────────────────────────────────────
    console.log('\n[WEB] 4. Trucks');
    await page.goto(`${WEB_URL}/carrier/trucks`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);

    const trucksText = await safeText(page);
    r.trucksPage = hasText(trucksText, 'License Plate', 'Type', 'Capacity', 'Actions');
    ...
    // ── 5. TRUCK DETAIL ───────────────────────────────────────────────
    console.log('\n[WEB] 5. Truck Detail');
    const viewLink = await page.$('a:has-text("View"), a[href*="/carrier/trucks/"]');
    if (viewLink) {
      ...
      r.truckDetail = hasText(truckDetailText, 'License Plate', 'Truck Type', 'Capacity', 'Status');
    ...
    await page.goto(`${WEB_URL}/carrier/trucks/add`, { waitUntil: 'domcontentloaded' });
    await sleep(2500);

    const addFormText = await safeText(page);
    r.truckAddForm = hasText(addFormText, 'License', 'Truck', 'Plate', 'Type', 'Capacity', 'Register');
    console.log(`   Add form: ${r.truckAddForm ? '✓' : '✗'}`);
    await screenshot(page, 'carrier-06d-truck-add-form');

    // ── 7. TRIPS ──────────────────────────────────────────────────────
    console.log('\n[WEB] 7. Trips');
    await page.goto(`${WEB_URL}/carrier/trips`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);

    const tripsText = await safeText(page);
    r.tripsPage = hasText(tripsText, 'Ready to Start', 'Load', 'Route', 'Truck', 'Status', 'Actions');
    ...
    const activeTripsClicked = await tryClick(page, [
      'button:has-text("Active Trips")',
      'a:has-text("Active Trips")',
    ], 'Active Trips tab');
    ...
    await tryClick(page, [
      'button:has-text("Ready to Start")',
      'a:has-text("Ready to Start")',
    ], 'Back to Ready to Start');
    await sleep(2000);

    // ── 8. TRIP DETAIL ────────────────────────────────────────────────
    console.log('\n[WEB] 8. Trip Detail');
    const tripLink = await page.$('a[href*="/carrier/trips/"], button:has-text("Details"), a:has-text("Details"), a:has-text("View")');
    if (tripLink) {
      ...
      r.tripDetail = hasText(tripDetailText, 'Status', 'Route', 'Truck', '→', 'Load');
      ...
      await tryClick(page, ['button:has-text("Active Trips")'], 'Active Trips (for detail)');
      await sleep(2000);
      const activeTripLink = await page.$('a[href*="/carrier/trips/"], button:has-text("Details"), a:has-text("Details")');
      if (activeTripLink) {
        await activeTripLink.click();
        await sleep(3000);
        const tripDetailText = await safeText(page);
        r.tripDetail = hasText(tripDetailText, 'Status', 'Route', 'Truck', '→', 'Load');
        console.log(`   Trip detail (from active): ${r.tripDetail ? '✓' : '✗'}`);
        await screenshot(page, 'carrier-07c-trip-detail');
        await page.goBack();
        await sleep(1500);
      } else {
    ...
    // ── 9. TRIP HISTORY ───────────────────────────────────────────────
    console.log('\n[WEB] 9. Trip History');
    await page.goto(`${WEB_URL}/carrier/trip-history`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);

    const tripHistText = await safeText(page);
    r.tripHistory = hasText(tripHistText, 'Trip', 'History', 'Completed', 'Route', 'Delivered');
    console.log(`   Trip history: ${r.tripHistory ? '✓' : '✗'}`);
    await screenshot(page, 'carrier-08-trip-history');

    // ── 10. MAP ───────────────────────────────────────────────────────
    console.log('\n[WEB] 10. Map');
    await page.goto(`${WEB_URL}/carrier/map`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);
    ...
    // ── 11. MATCHES ───────────────────────────────────────────────────
    console.log('\n[WEB] 11. Matches');
    await page.goto(`${WEB_URL}/carrier/matches`, { waitUntil: 'domcontentloaded' });
    await sleep(2500);
    ...
    // ── 12. REQUESTS ──────────────────────────────────────────────────
    console.log('\n[WEB] 12. Requests');
    await page.goto(`${WEB_URL}/carrier/requests`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);
    ...
    const loadReqClicked = await tryClick(page, [
      'button:has-text("My Load Requests")',
      'a:has-text("My Load Requests")',
    ...
    const proposalsClicked = await tryClick(page, [
      'button:has-text("Match Proposals")',
      'a:has-text("Match Proposals")',
    ], 'Match Proposals tab');
    if (proposalsClicked) {
      await sleep(2000);
    ...
    // ── 13. WALLET ────────────────────────────────────────────────────
    console.log('\n[WEB] 13. Wallet');
    await page.goto(`${WEB_URL}/carrier/wallet`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);
    ...
    // ── 14. DOCUMENTS ─────────────────────────────────────────────────
    console.log('\n[WEB] 14. Documents');
    await page.goto(`${WEB_URL}/carrier/documents`, { waitUntil: 'domcontentloaded' });
    await sleep(2500);

    const docsText = await safeText(page);
    r.documentsPage = hasText(docsText, 'Document', 'Upload', 'Insurance', 'License');
    console.log(`   Documents page: ${r.documentsPage ? '✓' : '✗'}`);
    await screenshot(page, 'carrier-13-documents');

    // ── 15. SETTINGS ──────────────────────────────────────────────────
    console.log('\n[WEB] 15. Settings');
    await page.goto(`${WEB_URL}/carrier/settings`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);

    const settingsText = await safeText(page);
    r.settingsPage = hasText(settingsText, 'Settings', 'Profile', 'Company', 'Account', 'Name', 'Email');
    console.log(`   Settings page: ${r.settingsPage ? '✓' : '✗'}`);
    await screenshot(page, 'carrier-14-settings');

    // ── 16. TEAM ──────────────────────────────────────────────────────
    console.log('\n[WEB] 16. Team');
    await page.goto(`${WEB_URL}/carrier/team`, { waitUntil: 'domcontentloaded' });
    await sleep(2500);
    ...
    // ── 17. GPS ───────────────────────────────────────────────────────
    console.log('\n[WEB] 17. GPS');
    await page.goto(`${WEB_URL}/carrier/gps`, { waitUntil: 'domcontentloaded' });
    await sleep(2500);
...
function printSummaryTable() {
  ...
  console.log('');
  console.log(`Total: ${pass} passed, ${fail} failed, ${features.length - pass - fail} skipped`);
}
...
async function main() {
  ...
  console.log('║  Comprehensive Carrier Browser Test — Web Portal                 ║');
  ...
  // Save results
  writeFileSync(`${SCREENSHOT_DIR}/carrier-results.json`, JSON.stringify(results, null, 2));
