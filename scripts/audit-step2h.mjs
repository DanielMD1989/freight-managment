...
const wallet = await prisma.financialAccount.findFirst({
  where: { organizationId: user.organizationId, accountType: "CARRIER_WALLET" },
});
const dbBalance = Number(wallet.balance);
const txEntries = await prisma.journalEntry.findMany({
  where: { lines: { some: { accountId: wallet.id } } },
  include: { lines: { where: { accountId: wallet.id } } },
  orderBy: { createdAt: "desc" },
});
console.log("DB balance:", dbBalance);
console.log("DB transaction entries:", txEntries.length);
for (const e of txEntries) {
  const l = e.lines[0];
  console.log(`  ${l.isDebit ? "-" : "+"}${Number(l.amount)} | ${e.transactionType} | "${e.description}"`);
}
...
console.log("Navigating to (carrier)/wallet...");
await page.goto("http://localhost:8088/(carrier)/wallet", { waitUntil: "domcontentloaded" });
await page.waitForTimeout(7000);
...
console.log("\nAll leaf texts on the rendered Expo wallet screen:");
