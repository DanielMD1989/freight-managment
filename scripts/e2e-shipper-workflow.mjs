...
// ─── API Helpers ──────────────────────────────────────────────────────────────
async function apiCall(method, path, { body, token, headers: extraHeaders } = {}) {
...
async function apiLogin(email, password) {
  ...
  if (status !== 200) throw new Error(`Login failed for ${email}: ${data?.error || status}`);
  return {
...
async function apiLoginWithCSRF(apiContext, email, password) {
  ...
  if (loginRes.status() !== 200) {
    ...
    throw new Error(`CSRF login failed for ${email}: ${body?.error || loginRes.status()}`);
  }
...
async function phase1(browser, state) {
  ...
  try {
    ...
    await fillField(
      ...
      SHIPPER_A.firstName, 'firstName'
    );
    await fillField(
      ['#lastName', 'input[name="lastName"]'],
      SHIPPER_A.lastName, 'lastName'
    );
    await fillField(
      ['#email', 'input[name="email"]', 'input[type="email"]'],
      SHIPPER_A.email, 'email'
    );
    await fillField(
      ['#phone', 'input[name="phone"]', 'input[type="tel"]'],
      SHIPPER_A.phone, 'phone'
    );
    ...
    await fillField(
      ...
      SHIPPER_A.companyName, 'companyName'
    );

    // Password fields come after company name in the form
    await fillField(
      ['#password', 'input[name="password"]'],
      SHIPPER_A.password, 'password'
    );
    await fillField(
      ['#confirmPassword', 'input[name="confirmPassword"]'],
      SHIPPER_A.password, 'confirm password'
    );
    ...
    const regSuccess = !afterRegUrl.includes('/register') || afterRegUrl.includes('/shipper') || afterRegUrl.includes('/dashboard');
    if (!regSuccess) {
      // Capture any error message shown on the page
      const pageText = await page.textContent('body').catch(() => '');
      const errorMatch = pageText.match(/(User with.*?exists|Validation error.*?\n|Password does not.*?\n|Too many registration.*?\n)/i);
      if (errorMatch) console.log(`   ⚠ Form error: ${errorMatch[0].trim()}`);
    }
    record('Phase 1', '1a. Register Shipper A (browser)', regSuccess ? 'PASS' : 'FAIL', afterRegUrl);
  ...
  try {
    ...
    record('Phase 1', '1b. Register Shipper B (API)', ok ? 'PASS' : 'FAIL', `status=${status}`);
  } catch (e) {
  ...
  try {
    ...
    record('Phase 1', '1c-i. Login returns limitedAccess=true', limitedOk ? 'PASS' : 'FAIL',
    ...
    const loadRes = await apiCall('POST', '/api/loads', {
      body: {
        pickupCity: 'Addis Ababa', deliveryCity: 'Dire Dawa',
        pickupDate: futureDate(7), deliveryDate: futureDate(10),
        truckType: 'FLATBED', weight: 15000,
    ...
    record('Phase 1', '1c-ii. POST /api/loads blocked (not 201)', blocked ? 'PASS' : 'FAIL',
      `status=${loadRes.status}`);

    // Browser is already logged in from registration — navigate to dashboard
    await page.goto(`${BASE_URL}/shipper/dashboard`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);
  ...
  try {
    ...
    const searchA = await apiCall('GET', `/api/admin/users?search=${encodeURIComponent(SHIPPER_A.email)}`, {
    ...
    if (userA) {
      ...
      record('Phase 1', '1d-i. Activate Shipper A', activateA.ok ? 'PASS' : 'FAIL',
        `status=${activateA.status}`);

      // Verify organization
      if (userA.organizationId) {
        const verifyOrgA = await apiCall('POST', `/api/admin/organizations/${userA.organizationId}/verify`, {
          token: state.admin.token,
        });
        record('Phase 1', '1d-ii. Verify Shipper A org', verifyOrgA.ok ? 'PASS' : 'FAIL',
          `status=${verifyOrgA.status}`);
      }
    } else {
      record('Phase 1', '1d-i. Find Shipper A', 'FAIL', 'User not found in admin search');
    }

    // Find and activate Shipper B
    const searchB = await apiCall('GET', `/api/admin/users?search=${encodeURIComponent(SHIPPER_B.email)}`, {
      token: state.admin.token,
    });
    ...
    if (userB) {
      ...
      record('Phase 1', '1d-iii. Activate Shipper B', activateB.ok ? 'PASS' : 'FAIL',
        `status=${activateB.status}`);

      if (userB.organizationId) {
        const verifyOrgB = await apiCall('POST', `/api/admin/organizations/${userB.organizationId}/verify`, {
          token: state.admin.token,
        });
        record('Phase 1', '1d-iv. Verify Shipper B org', verifyOrgB.ok ? 'PASS' : 'FAIL',
          `status=${verifyOrgB.status}`);
      }
    } else {
      record('Phase 1', '1d-iii. Find Shipper B', 'FAIL', 'User not found in admin search');
    }
  } catch (e) {
  ...
  try {
    ...
    record('Phase 1', '1e-i. Re-login returns limitedAccess=false', fullAccess ? 'PASS' : 'FAIL',
    ...
    record('Phase 1', '1e-ii. POST /api/loads succeeds (201)', canCreate ? 'PASS' : 'FAIL',
    ...
    record('Phase 1', '1e-iii. Full access verified via API', 'PASS', 'tokens refreshed');
  } catch (e) {
...
async function phase2(state) {
  ...
  try {
    const { csrfToken } = await apiLoginWithCSRF(apiCtxA, SHIPPER_A.email, SHIPPER_A.password);
    ...
    const licenseRes = await apiCtxA.post(`${BASE_URL}/api/documents/upload`, {
      multipart: {
        file: { name: 'company-license.pdf', mimeType: 'application/pdf', buffer: pdfBuffer },
    ...
    const licenseOk = licenseRes.status() === 200 && licenseData.document?.verificationStatus === 'PENDING';
    record('Phase 2', '2a-i. Upload COMPANY_LICENSE (PDF)', licenseOk ? 'PASS' : 'FAIL',
    ...
    const tinRes = await apiCtxA.post(`${BASE_URL}/api/documents/upload`, {
      multipart: {
        file: { name: 'tin-certificate.jpg', mimeType: 'image/jpeg', buffer: jpegBuffer },
    ...
    const tinOk = tinRes.status() === 200 && tinData.document?.verificationStatus === 'PENDING';
    record('Phase 2', '2a-ii. Upload TIN_CERTIFICATE (JPEG)', tinOk ? 'PASS' : 'FAIL',
    ...
    const insRes = await apiCtxA.post(`${BASE_URL}/api/documents/upload`, {
      multipart: {
        file: { name: 'insurance.pdf', mimeType: 'application/pdf', buffer: pdfBuffer },
    ...
    const insOk = insRes.status() === 200 && insData.document?.verificationStatus === 'PENDING';
    record('Phase 2', '2a-iii. Upload INSURANCE_CERTIFICATE', insOk ? 'PASS' : 'FAIL',
  ...
  try {
    const { csrfToken } = await apiLoginWithCSRF(apiCtxB, SHIPPER_B.email, SHIPPER_B.password);
    const orgId = state.shipperB.orgId;

    if (!orgId) throw new Error('Shipper B orgId not set');

    for (const docType of ['COMPANY_LICENSE', 'TIN_CERTIFICATE', 'INSURANCE_CERTIFICATE']) {
      const isInsurance = docType === 'INSURANCE_CERTIFICATE';
      const isPdf = docType !== 'TIN_CERTIFICATE';
      const multipart = {
        file: {
          name: isPdf ? `${docType.toLowerCase()}.pdf` : `${docType.toLowerCase()}.jpg`,
      ...
      record('Phase 2', `2b. Upload ${docType} (Shipper B)`, res.status() === 200 ? 'PASS' : 'FAIL',
        `status=${res.status()}`);
    }
  } catch (e) {
  ...
  console.log('\n[2c] Verify Document List');
  try {
    for (const [label, shipperState] of [['Shipper A', state.shipperA], ['Shipper B', state.shipperB]]) {
      const res = await apiCall('GET',
        `/api/documents?entityType=company&entityId=${shipperState.orgId}`,
        { token: shipperState.token },
      );
      const docs = res.data?.documents || [];
      const allPending = docs.length > 0 && docs.every(d => d.verificationStatus === 'PENDING');
      record('Phase 2', `2c. ${label} docs listed (count=${docs.length})`,
  ...
  try {
    ...
    const pendingRes = await apiCall('GET', '/api/admin/documents?status=PENDING', {
      token: state.admin.token,
    });
    ...
    const testOrgIds = [state.shipperA.orgId, state.shipperB.orgId].filter(Boolean);
    const testDocs = pendingDocs.filter(d =>
      testOrgIds.includes(d.organizationId) || testOrgIds.includes(d.organization?.id)
    );
    console.log(`   ${testDocs.length} belong to test shippers`);

    let approvedCount = 0;
    for (const doc of testDocs) {
      const approveRes = await apiCall('PATCH', `/api/admin/verification/${doc.id}`, {
        body: { entityType: doc.entityType || 'company', verificationStatus: 'APPROVED' },
        token: state.admin.token,
      });
      if (approveRes.ok) approvedCount++;
    }
    record('Phase 2', '2d. Admin approves all documents',
...
async function phase3(browser, state) {
  ...
  try {
    ...
    await sleep(1500);
    await page.fill('input[name="email"], input[type="email"]', SHIPPER_A.email);
    await page.fill('input[name="password"], input[type="password"]', SHIPPER_A.password);
    await page.click('button[type="submit"]');
    await sleep(4000);

    // Navigate to create load page
    await page.goto(`${BASE_URL}/shipper/loads/create`, { waitUntil: 'domcontentloaded' });
    await sleep(2500);
    ...
    // Click Next → Step 2
    let nextClicked = await page.$('button:has-text("Next"), button:has-text("Continue")');
    if (nextClicked) { await nextClicked.click(); await sleep(1500); }
    ...
    if (flatbedChip) { await flatbedChip.click(); console.log('   ✓ Selected Flatbed truck type'); }

    const weightInput = await page.$('input[type="number"][placeholder*="5000"], input[type="number"]');
    if (weightInput) { await weightInput.fill('15000'); console.log('   ✓ Set weight: 15000'); }

    const cargoInput = await page.$('textarea');
    if (cargoInput) { await cargoInput.fill('Coffee beans for export'); console.log('   ✓ Set cargo description'); }

    await screenshot(page, '03a-create-step2-filled');

    // Click Next → Step 3
    nextClicked = await page.$('button:has-text("Next"), button:has-text("Continue")');
    if (nextClicked) { await nextClicked.click(); await sleep(1500); }
    ...
    if (!state.shipperA.loadId) {
      const myLoads = await apiCall('GET', '/api/loads?myLoads=true&status=POSTED', {
        token: state.shipperA.token,
      });
      const loads = myLoads.data?.loads || [];
      if (loads.length > 0) {
        // Find the Addis Ababa → Dire Dawa load
        const ourLoad = loads.find(l => l.deliveryCity === 'Dire Dawa') || loads[0];
        state.shipperA.loadId = ourLoad.id;
        console.log(`   Load ID from API: ${state.shipperA.loadId}`);
      }
    }

    record('Phase 3', '3a. Shipper A posts load (web wizard)',
      state.shipperA.loadId ? 'PASS' : 'FAIL',
      `loadId=${state.shipperA.loadId || 'NOT_FOUND'}`);
  } catch (e) {
    record('Phase 3', '3a. Shipper A posts load (web wizard)', 'FAIL', e.message);
  ...
  try {
    ...
    if (!state.shipperA.loadId) {
      const fallbackRes = await apiCall('POST', '/api/loads', {
        body: {
          ...
          pickupDate: futureDate(7), deliveryDate: futureDate(10),
          truckType: 'FLATBED', weight: 15000,
          cargoDescription: 'Coffee beans for export', status: 'POSTED',
        },
        token: state.shipperA.token,
      });
      if (fallbackRes.ok) {
        state.shipperA.loadId = fallbackRes.data?.load?.id;
        record('Phase 3', '3a-fallback. Shipper A load via API', 'PASS',
    ...
    const loadBRes = await apiCall('POST', '/api/loads', {
      body: {
        pickupCity: 'Addis Ababa', deliveryCity: 'Mekelle',
        pickupDate: futureDate(7), deliveryDate: futureDate(10),
        truckType: 'FLATBED', weight: 20000,
        cargoDescription: 'Construction materials - cement bags', status: 'POSTED',
      },
      token: state.shipperB.token,
    });
    const loadBOk = loadBRes.status === 201;
    state.shipperB.loadId = loadBRes.data?.load?.id;
    record('Phase 3', '3b. Shipper B posts load (API)', loadBOk ? 'PASS' : 'FAIL',
  ...
  try {
    ...
    state.carrier = { token: carrierLogin.sessionToken, orgId: carrierLogin.user.organizationId };
    ...
    const seesA = state.shipperA.loadId && allLoads.some(l => l.id === state.shipperA.loadId);
    const seesB = state.shipperB.loadId && allLoads.some(l => l.id === state.shipperB.loadId);
    record('Phase 3', '3c. Carrier sees both loads', seesA && seesB ? 'PASS' : 'FAIL',
  ...
  try {
    ...
    const dispSeesA = state.shipperA.loadId && dispAll.some(l => l.id === state.shipperA.loadId);
    const dispSeesB = state.shipperB.loadId && dispAll.some(l => l.id === state.shipperB.loadId);
    record('Phase 3', '3d. Dispatcher sees both loads', dispSeesA && dispSeesB ? 'PASS' : 'FAIL',
...
async function phase4(state) {
  ...
  try {
    ...
    const postingsRes = await apiCall('GET', '/api/truck-postings?status=ACTIVE', {
      token: state.shipperA.token,
    });
    const allPostings = postingsRes.data?.truckPostings || postingsRes.data?.postings || [];
    ...
    const carrierPostings = allPostings.filter(p => myTruckIds.has(p.truckId || p.truck?.id));
    console.log(`   ${carrierPostings.length} belong to test carrier`);

    const postings = carrierPostings.length >= 2 ? carrierPostings : allPostings;
    if (postings.length < 2) {
      record('Phase 4', '4a. Find available trucks', 'FAIL',
        `Need ≥2 carrier postings, found ${postings.length}`);
      return;
    }

    // Use truckId (direct FK) or fall back to truck.id (nested relation)
    state.truck1 = { id: postings[0].truckId || postings[0].truck?.id, postingId: postings[0].id };
    state.truck2 = { id: postings[1].truckId || postings[1].truck?.id, postingId: postings[1].id };
    record('Phase 4', '4a. Find available trucks', 'PASS',
  ...
  console.log('\n[4b] Shipper A Sends Truck Request');
  try {
    if (!state.shipperA.loadId || !state.truck1.id) throw new Error('Missing loadId or truckId');
    ...
    record('Phase 4', '4b. Shipper A truck request', reqOk ? 'PASS' : 'FAIL',
  ...
  try {
    ...
    const pendingRes = await apiCall('GET', '/api/truck-requests?status=PENDING', {
      token: state.carrier.token,
    });
    const pending = pendingRes.data?.requests || [];
    const ourReq = pending.find(r => r.loadId === state.shipperA.loadId) || pending[0];

    if (!ourReq) throw new Error('No pending request found for carrier');

    const approveRes = await apiCall('POST', `/api/truck-requests/${ourReq.id}/respond`, {
      body: { action: 'APPROVE' },
      token: state.carrier.token,
    });
    ...
    record('Phase 4', '4c-i. Carrier approves request', approveOk ? 'PASS' : 'FAIL',
      `tripId=${tripId?.slice(-8)}, tripStatus=${tripStatus}`);

    // Verify truck1 posting is now MATCHED (no longer in public listings)
    const postingsAfter = await apiCall('GET', '/api/truck-postings?status=ACTIVE', {
      token: state.shipperA.token,
    });
    const activePostings = postingsAfter.data?.truckPostings || postingsAfter.data?.postings || [];
    const truck1StillActive = activePostings.some(p =>
      (p.truckId || p.truck?.id) === state.truck1.id
    );
    record('Phase 4', '4c-ii. Truck1 removed from marketplace', !truck1StillActive ? 'PASS' : 'FAIL',
  ...
  try {
    if (!state.shipperB.loadId || !state.truck2.id) throw new Error('Missing loadId or truckId');
    ...
    record('Phase 4', '4d. Shipper B truck request', reqBRes.status === 201 ? 'PASS' : 'FAIL',
  ...
  try {
    ...
    const pending = pendingRes.data?.requests || [];
    const reqB = pending.find(r => r.loadId === state.shipperB.loadId) || pending[0];

    if (!reqB) throw new Error('No pending request found for Shipper B');

    const rejectRes = await apiCall('POST', `/api/truck-requests/${reqB.id}/respond`, {
      body: { action: 'REJECT', responseNotes: 'Truck not available for this route' },
      token: state.carrier.token,
    });

    const rejectOk = rejectRes.ok;
    const noTrip = !rejectRes.data?.trip;
    record('Phase 4', '4e-i. Carrier rejects request', rejectOk ? 'PASS' : 'FAIL',
      `noTrip=${noTrip}`);

    // Verify truck2 still ACTIVE in marketplace
    const postingsAfter2 = await apiCall('GET', '/api/truck-postings?status=ACTIVE', {
      token: state.shipperA.token,
    });
    const activePostings2 = postingsAfter2.data?.truckPostings || postingsAfter2.data?.postings || [];
    const truck2Active = activePostings2.some(p =>
      (p.truckId || p.truck?.id) === state.truck2.id
    );
    record('Phase 4', '4e-ii. Truck2 still on marketplace', truck2Active ? 'PASS' : 'FAIL',
  ...
  try {
    ...
    record('Phase 4', '4f. Request statuses', approved >= 1 && rejected >= 1 ? 'PASS' : 'FAIL',
...
async function phase5(browser, state) {
  ...
  if (!state.tripA?.id) {
    record('Phase 5', '5. SKIP — no trip from Phase 4', 'FAIL', 'tripId missing');
    return;
  }
  ...
  const transitions = [
    { status: 'PICKUP_PENDING', timestampField: 'startedAt', label: 'ASSIGNED → PICKUP_PENDING' },
    { status: 'IN_TRANSIT', timestampField: 'pickedUpAt', label: 'PICKUP_PENDING → IN_TRANSIT' },
    {
      status: 'DELIVERED', timestampField: 'deliveredAt', label: 'IN_TRANSIT → DELIVERED',
  ...
  for (const t of transitions) {
    try {
      ...
      record('Phase 5', `5a. ${t.label}`, ok && tsSet ? 'PASS' : ok ? 'WARN' : 'FAIL',
  ...
  console.log('\n[5b] Carrier Uploads POD — Browser');
  const podCtx = await browser.newContext({ viewport: { width: 1280, height: 900 } });
  ...
  try {
    ...
    await sleep(1500);
    await podPage.fill('input[name="email"], input[type="email"]', CARRIER.email);
    await podPage.fill('input[name="password"], input[type="password"]', CARRIER.password);
    ...
    await podPage.goto(`${BASE_URL}/carrier/trips/${tripId}`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);
    await screenshot(podPage, '05b-trip-detail-delivered');
    record('Phase 5', '5b-ii. Carrier trip detail page', 'PASS', `tripId=${tripId}`);
    ...
    // Fill notes
    await podPage.fill('input[placeholder="e.g., Signed by warehouse manager"]', 'Delivered in good condition, all 15 tons received');
    await sleep(500);
    ...
    record('Phase 5', '5b-iv. POD file uploaded', podUploaded ? 'PASS' : 'WARN',
    ...
    record('Phase 5', '5b. Carrier uploads POD (browser)', 'PASS', 'visible upload flow completed');
  } catch (e) {
  ...
  const confirmCtx = await browser.newContext({ viewport: { width: 1280, height: 900 } });
  const confirmPage = await confirmCtx.newPage();
  confirmPage.setDefaultTimeout(20000);
  try {
    // Login as Shipper A in browser
    await confirmPage.goto(`${BASE_URL}/login`, { waitUntil: 'domcontentloaded' });
    await sleep(1500);
    await confirmPage.fill('input[name="email"], input[type="email"]', SHIPPER_A.email);
    await confirmPage.fill('input[name="password"], input[type="password"]', SHIPPER_A.password);
    ...
    // Navigate to trip detail page
    await confirmPage.goto(`${BASE_URL}/shipper/trips/${tripId}`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);
    await screenshot(confirmPage, '05c-trip-detail-pod-ready');
    record('Phase 5', '5c-ii. Shipper trip detail (POD submitted)', 'PASS', `tripId=${tripId}`);

    // Verify the "Confirm Delivery Receipt" alert is visible
    const confirmAlert = await confirmPage.$('text=Confirm Delivery Receipt');
    record('Phase 5', '5c-iii. Delivery confirmation alert visible',
      confirmAlert ? 'PASS' : 'WARN', `alertVisible=${!!confirmAlert}`);
    ...
    // Fill optional notes
    await confirmPage.fill('textarea[placeholder="Any notes about the delivery..."]',
    ...
    record('Phase 5', '5c-v. Delivery confirmed & trip completed', 'PASS', 'browser confirmation flow completed');
    ...
    record('Phase 5', '5c. Shipper confirms delivery (browser)', 'FAIL', e.message);
  }
  ...
  try {
    ...
    record('Phase 5', '5d-i. Trip COMPLETED', isCompleted ? 'PASS' : 'FAIL',
    ...
    record('Phase 5', '5d-ii. Load POD verified & synced', podVerified ? 'PASS' : 'WARN',
  ...
  try {
    ...
    const postings5e = postingsRes.data?.truckPostings || postingsRes.data?.postings || [];
    const truck1Back = postings5e.some(p =>
      (p.truckId || p.truck?.id) === state.truck1?.id
    );
    record('Phase 5', '5e. Truck1 back on marketplace', truck1Back ? 'PASS' : 'WARN',
  ...
  try {
    ...
    const shipperBalance = shipperWallet.data?.wallet?.balance ?? shipperWallet.data?.totalBalance ?? shipperWallet.data?.balance;
    const shipperCurrency = shipperWallet.data?.wallet?.currency ?? shipperWallet.data?.currency ?? 'ETB';
    const carrierBalance = carrierWallet.data?.wallet?.balance ?? carrierWallet.data?.totalBalance ?? carrierWallet.data?.balance;
    const carrierCurrency = carrierWallet.data?.wallet?.currency ?? carrierWallet.data?.currency ?? 'ETB';
    record('Phase 5', '5f-i. Shipper A wallet', shipperWallet.ok ? 'PASS' : 'WARN',
      `balance=${shipperBalance} ${shipperCurrency}`);
    record('Phase 5', '5f-ii. Carrier wallet', carrierWallet.ok ? 'PASS' : 'WARN',
  ...
  try {
    ...
    await pg.fill('input[name="password"], input[type="password"]', CARRIER.password);
    await pg.click('button[type="submit"]');
    await sleep(4000);

    await pg.goto(`${BASE_URL}/carrier/requests`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);
...
async function phase6(browser, state) {
  console.log('\n' + '═'.repeat(70));
  console.log('  PHASE 6: Browser Visual Tests — Trip Accepted (Mobile) + Rejected (Web)');
  console.log('═'.repeat(70));
  ...
  try {
    const mobileCheck = await fetch(MOBILE_URL, { signal: AbortSignal.timeout(5000) });
    mobileAvailable = mobileCheck.ok || mobileCheck.status < 500;
  } catch { /* not running */ }

  if (!mobileAvailable) {
    record('Phase 6', '6a-i. Mobile login as Shipper A', 'WARN',
  ...
  if (mobileAvailable) try {
    // Inject auth tokens directly into localStorage (avoids rate-limited /api/auth/login)
    await mobilePage.goto(MOBILE_URL, { waitUntil: 'domcontentloaded', timeout: 30000 });
    await mobilePage.evaluate(({ token, userId, role }) => {
    ...
    }, { token: state.shipperA.token, userId: state.shipperA.userId, role: 'SHIPPER' });
    console.log('   ✓ Auth tokens injected into localStorage (skipping UI login)');
    ...
    record('Phase 6', '6a-i. Mobile auth via token injection', loginOk ? 'PASS' : 'FAIL',
      `url=${afterUrl}`);
    ...
    if (loginOk) {
      ...
      const hasDashStats = dashBody.includes('Active') || dashBody.includes('Loads')
        || dashBody.includes('Shipments') || dashBody.includes('Transit');
      await screenshot(mobilePage, '06a-mobile-dashboard');
      record('Phase 6', '6a-ii. Mobile dashboard loaded', hasDashStats ? 'PASS' : 'WARN',
        `hasStats=${hasDashStats}`);

      // ── Loads List ─────────────────────────────────────────────────────
      const loadsTabClicked = await navigateToTab(mobilePage, '/loads', 'Loads');
      await sleep(3000);
      await screenshot(mobilePage, '06a-mobile-loads-list');

      const loadsBody = await mobilePage.textContent('body').catch(() => '');
      const hasLoadRoute = loadsBody.includes('Dire Dawa') || loadsBody.includes('Addis Ababa');
      record('Phase 6', '6a-iii. Mobile loads list', loadsTabClicked && hasLoadRoute ? 'PASS' : 'WARN',
        `tabClicked=${loadsTabClicked}, hasRoute=${hasLoadRoute}`);

      // ── Load Detail ────────────────────────────────────────────────────
      if (hasLoadRoute) {
        const loadCardClicked = await tryClick(mobilePage, [
        ...
        if (loadCardClicked) {
          ...
          const loadDetailBody = await mobilePage.textContent('body').catch(() => '');
          const hasLoadDetail = loadDetailBody.includes('Dire Dawa') || loadDetailBody.includes('Addis Ababa');
          record('Phase 6', '6a-iv. Mobile load detail', hasLoadDetail ? 'PASS' : 'WARN',
      ...
      const tripsTabClicked = await navigateToTab(mobilePage, '/trips', 'Shipments');
      await sleep(3000);
      await screenshot(mobilePage, '06a-mobile-trips-list');

      const tripsBody = await mobilePage.textContent('body').catch(() => '');
      const hasTripRoute = tripsBody.includes('Addis Ababa') && tripsBody.includes('Dire Dawa');
      const hasCompletedStatus = tripsBody.includes('COMPLETED') || tripsBody.includes('Completed');
      record('Phase 6', '6a-v. Mobile trips list', tripsTabClicked && hasTripRoute ? 'PASS' : 'WARN',
        `hasRoute=${hasTripRoute}, hasCompleted=${hasCompletedStatus}`);

      // ── Trip Detail ────────────────────────────────────────────────────
      if (hasTripRoute) {
        const tripCardClicked = await tryClick(mobilePage, [
        ...
        if (tripCardClicked) {
          ...
          const detailBody = await mobilePage.textContent('body').catch(() => '');
          const hasDetailRoute = detailBody.includes('Addis Ababa') && detailBody.includes('Dire Dawa');
          const hasStatusBadge = detailBody.includes('COMPLETED') || detailBody.includes('Completed')
            || detailBody.includes('DELIVERED') || detailBody.includes('Delivered');
          const hasShipmentDetails = detailBody.includes('Shipment Details') || detailBody.includes('Distance');
          const hasCarrierInfo = detailBody.includes('Carrier');
          const hasReceiverInfo = detailBody.includes('Dawit') || detailBody.includes('Receiver');
          const hasPodSection = detailBody.includes('Proof of Delivery') || detailBody.includes('POD');
          const hasCompletedBanner = detailBody.includes('Trip Completed');

          record('Phase 6', '6a-vi. Mobile trip detail',
          ...
          record('Phase 6', '6a-vi. Mobile trip detail', 'WARN', 'Could not click trip card');
        }
      }

      // ── Requests ───────────────────────────────────────────────────────
      console.log('\n[6a-requests] Mobile Shipper A — Requests');
      await mobilePage.goto(`${MOBILE_URL}/requests`, { waitUntil: 'domcontentloaded' });
      await sleep(4000);
      await screenshot(mobilePage, '06a-mobile-requests');

      const reqBody = await mobilePage.textContent('body').catch(() => '');
      const hasRequests = reqBody.includes('Truck Request') || reqBody.includes('Carrier Request')
        || reqBody.includes('APPROVED') || reqBody.includes('Approved');
      record('Phase 6', '6a-vii. Mobile requests page', hasRequests ? 'PASS' : 'WARN',
        `hasRequestContent=${hasRequests}`);

      // ── Wallet ─────────────────────────────────────────────────────────
      console.log('\n[6a-wallet] Mobile Shipper A — Wallet');
      await mobilePage.goto(`${MOBILE_URL}/wallet`, { waitUntil: 'domcontentloaded' });
      await sleep(4000);
      await screenshot(mobilePage, '06a-mobile-wallet');

      const walletBody = await mobilePage.textContent('body').catch(() => '');
      const hasWalletContent = walletBody.includes('Balance') || walletBody.includes('ETB')
        || walletBody.includes('Wallet') || walletBody.includes('Transaction');
      record('Phase 6', '6a-viii. Mobile wallet page', hasWalletContent ? 'PASS' : 'WARN',
  ...
  try {
    ...
    await sleep(2000);
    await webPage.fill('input[name="email"], input[type="email"]', SHIPPER_B.email);
    await webPage.fill('input[name="password"], input[type="password"]', SHIPPER_B.password);
    await webPage.click('button[type="submit"]');
    await sleep(4000);

    const webAfterUrl = webPage.url();
    const webLoginOk = !webAfterUrl.includes('/login');
    record('Phase 6', '6b-i. Web login as Shipper B', webLoginOk ? 'PASS' : 'FAIL',
      `url=${webAfterUrl}`);
    await screenshot(webPage, '06b-web-shipperB-dashboard');

    if (webLoginOk) {
      // Navigate to Requests page
      await webPage.goto(`${BASE_URL}/shipper/requests`, { waitUntil: 'domcontentloaded' });
      await sleep(3000);
      await screenshot(webPage, '06b-web-requests-all');

      const reqText = await webPage.textContent('body').catch(() => '');
      const hasRequestsPage = reqText.includes('Request') || reqText.includes('request');
      record('Phase 6', '6b-ii. Web requests page loaded', hasRequestsPage ? 'PASS' : 'FAIL',
        `hasRequestContent=${hasRequestsPage}`);

      // Click on REJECTED filter tab
      const rejectedClicked = await tryClick(webPage, [
      ...
      if (rejectedClicked) {
        ...
        const showsRejected = filteredText.includes('REJECTED') || filteredText.includes('Rejected');
        const showsRoute = filteredText.includes('Mekelle') || filteredText.includes('Addis Ababa');
        record('Phase 6', '6b-iii. REJECTED request visible',
        ...
        record('Phase 6', '6b-iii. REJECTED request visible (all tab)',
      ...
      await webPage.goto(`${BASE_URL}/shipper/trips`, { waitUntil: 'domcontentloaded' });
      await sleep(3000);
      ...
      const hasNoTrips = tripsText.includes('No trips') || tripsText.includes('no trip')
        || tripsText.includes('No shipments') || tripsText.includes('appear here')
        || !tripsText.includes('Mekelle');
      record('Phase 6', '6b-iv. Shipper B has no trips (rejected)', hasNoTrips ? 'PASS' : 'WARN',
        `noTrips=${hasNoTrips}`);

      // Navigate to loads — verify load is still POSTED (not assigned)
      await webPage.goto(`${BASE_URL}/shipper/loads`, { waitUntil: 'domcontentloaded' });
      await sleep(3000);
      await screenshot(webPage, '06b-web-shipperB-loads');

      const loadsText = await webPage.textContent('body').catch(() => '');
      const hasLoad = loadsText.includes('Mekelle') || loadsText.includes('POSTED');
      record('Phase 6', '6b-v. Shipper B load still visible', hasLoad ? 'PASS' : 'WARN',
        `hasLoad=${hasLoad}`);
    }
  } catch (e) {
  ...
  try {
    ...
    if (state.shipperACookies?.length) {
      ...
      console.log(`   ✓ Injected ${state.shipperACookies.length} saved cookies (skipping form login)`);
    } else {
      // Fallback: try form login (may be rate-limited)
      console.log('   ⚠ No saved cookies — attempting form login (may be rate-limited)');
      await webPage.goto(`${BASE_URL}/login`, { waitUntil: 'domcontentloaded' });
      await sleep(2000);
      await webPage.fill('input[name="email"], input[type="email"]', SHIPPER_A.email);
      await webPage.fill('input[name="password"], input[type="password"]', SHIPPER_A.password);
      await webPage.click('button[type="submit"]');
      await sleep(4000);
    }
    ...
    const hasTrip = tripsA.includes('Dire Dawa') || tripsA.includes('COMPLETED')
      || tripsA.includes('DELIVERED') || tripsA.includes('ASSIGNED');
    record('Phase 6', '6c-i. Shipper A trip visible on web', hasTrip ? 'PASS' : 'WARN',
      `hasTrip=${hasTrip}`);
    ...
    if (tripLink) {
      ...
      const hasDetail = detailText.includes('Dire Dawa') || detailText.includes('Trip Details')
        || detailText.includes('Shipment');
      record('Phase 6', '6c-ii. Trip detail page on web', hasDetail ? 'PASS' : 'WARN',
        `hasDetail=${hasDetail}`);
    }
    ...
    record('Phase 6', '6c-iii. Shipper A APPROVED request on web', hasApproved ? 'PASS' : 'WARN',
      `hasApproved=${hasApproved}`);

    // Navigate to Wallet — check balance after settlement
    await webPage.goto(`${BASE_URL}/shipper/wallet`, { waitUntil: 'domcontentloaded' });
    await sleep(3000);
    await screenshot(webPage, '06c-web-shipperA-wallet');

    const walletText = await webPage.textContent('body').catch(() => '');
    const hasWallet = walletText.includes('Wallet') || walletText.includes('Balance')
      || walletText.includes('ETB') || walletText.includes('Transaction');
    record('Phase 6', '6c-iv. Shipper A wallet on web', hasWallet ? 'PASS' : 'WARN',
      `hasWallet=${hasWallet}`);
...
function printReport() {
  ...
  for (const r of results) {
    const status = r.status === 'PASS' ? '✓ PASS'
      : r.status === 'FAIL' ? '✗ FAIL'
      : '⚠ WARN';
    console.log(`| ${r.phase.padEnd(7)} | ${r.test.padEnd(42)} | ${status.padEnd(7)} |`);
  }
  ...
  console.log(`Total: ${results.length} tests — ${pass} passed, ${fail} failed, ${warn} warnings`);
...
function saveResults() {
  ...
  writeFileSync(`${SCREENSHOT_DIR}/results.json`, JSON.stringify(report, null, 2));
...
async function main() {
  ...
  console.log('║  E2E Shipper Workflow Test — Complete Lifecycle                   ║');
