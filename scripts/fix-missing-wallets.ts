...
async function fixMissingWallets() {
  ...
  console.log(
    `⚠️  Found ${orgsWithoutWallets.length} organizations without wallets:\n`
  );
  ...
  for (const org of orgsWithoutWallets) {
    try {
      ...
      const walletType =
        org.type === "SHIPPER" ? "SHIPPER_WALLET" : "CARRIER_WALLET";
      ...
      console.error(
        `  ❌ Failed to create wallet for "${org.name}": ${error.message}`
      );
      failed++;
    }
  }
  ...
  if (failed === 0) {
    ...
    console.log(
      "⚠️  Some wallets could not be created. Check the errors above.\n"
    );
  }
}
