...
async function main() {
  ...
  const ctx = await browser.newContext({ viewport: { width: 390, height: 844 }, javaScriptEnabled: true });
  ...
  await page.waitForFunction(() => document.querySelectorAll('div').length > 20, { timeout: 30000 });
  await sleep(3000);

  // Skip onboarding
  const skipBtn = await page.$('div:has-text("Skip"):not(:has(div:has-text("Skip")))');
  if (skipBtn) { await skipBtn.click(); await sleep(2000); }
  ...
  await page.screenshot({ path: `${DIR}/mobile-tab-01-dashboard.png`, fullPage: true });

  // Scroll down to see more of dashboard
  await page.evaluate(() => window.scrollTo(0, 600));
  await sleep(1000);
  await page.screenshot({ path: `${DIR}/mobile-tab-01b-dashboard-scroll.png`, fullPage: true });

  // 2. My Loads tab
  console.log('\n2. My Loads tab');
  if (await clickBottomTab('My Loads')) {
    await page.screenshot({ path: `${DIR}/mobile-tab-02-my-loads.png`, fullPage: true });
    ...
    await sleep(1000);
    await page.screenshot({ path: `${DIR}/mobile-tab-02b-loads-scroll.png`, fullPage: true });
  }

  // 3. Shipments tab
  console.log('\n3. Shipments tab');
  if (await clickBottomTab('Shipment')) {
    await page.screenshot({ path: `${DIR}/mobile-tab-03-shipments.png`, fullPage: true });
    const txt = await page.textContent('body');
    console.log(`  Content: ${txt.substring(0, 300)}`);
  }
  ...
  if (await clickBottomTab('Find Trucks')) {
    await page.screenshot({ path: `${DIR}/mobile-tab-04-trucks.png`, fullPage: true });
    const txt = await page.textContent('body');
    console.log(`  Content: ${txt.substring(0, 300)}`);
    ...
    await page.evaluate(() => window.scrollTo(0, 400));
    await sleep(1000);
    await page.screenshot({ path: `${DIR}/mobile-tab-04b-trucks-scroll.png`, fullPage: true });
    ...
    for (const card of cards) {
      ...
      if (txt.includes('Book') || txt.includes('Request') || txt.includes('FLATBED') || txt.includes('DRY_VAN') || txt.includes('Flatbed')) {
        console.log(`  Clicking card: "${txt.substring(0, 60)}"`);
        await card.click();
        await sleep(2000);
        await page.screenshot({ path: `${DIR}/mobile-tab-05-booking.png`, fullPage: true });
        break;
      }
    }
  }
  ...
  if (await clickBottomTab('My Loads')) {
    ...
    for (const card of loadCards) {
      const txt = (await card.textContent().catch(() => '')).trim();
      if (txt.includes('Addis Ababa') || txt.includes('→') || txt.includes('POSTED') || txt.includes('COMPLETED')) {
        console.log(`  Clicking load: "${txt.substring(0, 80)}"`);
        await card.click();
        await sleep(3000);
        await page.screenshot({ path: `${DIR}/mobile-tab-06-load-detail.png`, fullPage: true });
