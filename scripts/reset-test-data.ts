...
async function main() {
  ...
  if (shippers.length === 0 || carriers.length === 0) {
    console.log(
      "ERROR: No shippers or carriers found. Please run seed script first."
    );
    return;
  }
  ...
  if (locations.length < 2) {
    console.log(
      "ERROR: Not enough locations found. Please seed locations first."
    );
    return;
  }
  ...
  // Count final data
  const totalPostings = await prisma.truckPosting.count({
    where: { status: "ACTIVE" },
  });
