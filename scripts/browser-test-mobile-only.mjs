  ...
  try {
    ...
    if (recentLoadsHeader) {
      ...
      for (const card of allCards) {
        ...
        if (t.includes('Recent Loads')) { foundHeader = true; continue; }
    ...
    if (recentLoad) {
      ...
      r.dashboardRecentLoadClick = url.includes('loads/') || url.includes('trips/')
        || bodyText.includes('Cargo') || bodyText.includes('Load Details') || bodyText.includes('Shipment Details');
    ...
    try {
      ...
      if (mInputs.length >= 4) {
        ...
        await mInputs[3].evaluate(el => el.dispatchEvent(new Event('blur', { bubbles: true })));
        await sleep(200);
      }
      r.createStep1 = true;
      await screenshot(page, 'mobile-07b-create-step1-filled');

      const mStep2 = await tryClick(page, [
      ...
      if (mStep2) {
        ...
        const mStep3 = await tryClick(page, [
          '[data-testid="create-load-next"]',
          'div[role="button"]:has-text("Next")',
          'button:has-text("Next")',
        ], 'Next → Step 3');
        if (mStep3) {
          ...
          const mStep4 = await tryClick(page, [
            '[data-testid="create-load-next"]',
            'div[role="button"]:has-text("Next")',
            'button:has-text("Next")',
          ], 'Next → Step 4 Review');
    ...
    try {
      ...
      r.requestsPage = hasText(reqBody, 'Request', 'Pending', 'Accepted', 'Booking', 'No requests');
    ...
    try {
      ...
      r.walletPage = hasText(walletBody, 'Wallet', 'Balance', 'ETB', 'Transaction', 'History', 'Payment');
    ...
    try {
      ...
      r.settingsPage = hasText(settingsBody, 'Settings', 'Profile', 'Language', 'Theme', 'Notification', 'Account', 'Logout');
    ...
    try {
      ...
      r.documentsPage = hasText(docsBody, 'Document', 'Upload', 'License', 'File', 'ID', 'No documents');
    ...
    try {
      ...
      r.matchesPage = hasText(matchesBody, 'Match', 'Truck', 'Load', 'Score', 'No matches');
...
async function main() {
  ...
  console.log('║  Mobile-Only Shipper Browser Test (Expo Web)                     ║');
