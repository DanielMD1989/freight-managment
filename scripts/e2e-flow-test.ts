...
function authHeaders(session: Session, includeCsrf = false): Record<string, string> {
...
async function main() {
  ...
  try {
    ...
    console.log(`  ✅ Shipper:    ${shipper.user.email} (org: ${shipper.user.organizationId})`);
    console.log(`  ✅ Carrier:    ${carrier.user.email} (org: ${carrier.user.organizationId})`);
    console.log(`  ✅ Dispatcher: ${dispatcher.user.email} (org: ${dispatcher.user.organizationId})`);
    console.log(`  ✅ Admin:      ${admin.user.email}`);
  } catch (err: any) {
    console.error(`\n❌ SETUP FAILED: ${err.message}`);
    console.error('   Make sure the dev server is running and seed data is loaded.');
  ...
  const initialShipperBalance = initialShipperWallet.data?.totalBalance ?? 'N/A';
  const initialCarrierBalance = initialCarrierWallet.data?.totalBalance ?? 'N/A';
  ...
  record({
    ...
    data: step1.data?.load
      ? { id: step1.data.load.id, status: step1.data.load.status, pickupCity: step1.data.load.pickupCity, deliveryCity: step1.data.load.deliveryCity }
      : step1.data,
    error: step1.status !== 201 ? (step1.data?.error || `Unexpected status ${step1.status}`) : undefined,
  });
  ...
  const step2 = await apiCall('GET', `/api/loads/${loadId}/matching-trucks`, shipper);
  const matchingTrucks = step2.data?.trucks || [];
  const matchedTruck = matchingTrucks.find((t: any) => t.truck?.truckType === 'DRY_VAN') || matchingTrucks[0];
  ...
  record({
    ...
    error: step2.status !== 200 ? step2.data?.error : matchingTrucks.length === 0 ? 'No matching trucks found' : undefined,
  });
  ...
  if (truckPostingId) {
    const step3Res = await apiCall('GET', `/api/truck-postings/${truckPostingId}/matching-loads`, carrier);
    ...
    step3Error = !ourLoad && step3Res.status === 200 ? 'Our test load not found in matching results' : step3Res.data?.error;
  } else {
    step3Status = 0;
  ...
  const step4 = await apiCall('POST', '/api/match-proposals', dispatcher, step4Body, true);
  ...
  record({
    ...
    data: step4.data?.proposal
      ...
      : step4.data,
    error: step4.status !== 201 ? (step4.data?.error || `Unexpected status ${step4.status}`) : undefined,
  });
  ...
  const step5 = await apiCall('POST', `/api/match-proposals/${proposalId}/respond`, carrier, step5Body);
  ...
  record({
    ...
    body: step5Body,
    status: step5.status,
    pass: step5.status === 200 && step5.data?.proposal?.status === 'ACCEPTED' && !!step5.data?.trip,
    data: {
    ...
    error: step5.status !== 200 ? (step5.data?.error || `Unexpected status ${step5.status}`) : undefined,
  });
  ...
  const step7 = await apiCall('PATCH', `/api/trips/${tripId}`, carrier, step7Body);
  ...
  record({
    ...
    error: step7.status !== 200 ? (step7.data?.error || `Unexpected status ${step7.status}`) : undefined,
  });

  // ─── Step 8: Carrier picks up: PICKUP_PENDING → IN_TRANSIT ──────────────

  const step8Body = { status: 'IN_TRANSIT' };
  const step8 = await apiCall('PATCH', `/api/trips/${tripId}`, carrier, step8Body);
  ...
  record({
    ...
    error: step8.status !== 200 ? (step8.data?.error || `Unexpected status ${step8.status}`) : undefined,
  });
  ...
  const step9 = await apiCall('PATCH', `/api/trips/${tripId}`, carrier, step9Body);
  ...
  record({
    ...
    error: step9.status !== 200 ? (step9.data?.error || `Unexpected status ${step9.status}`) : undefined,
  });

  // ─── Step 10: Attempt completion without POD (expect rejection) ─────────

  const step10Body = { status: 'COMPLETED' };
  const step10 = await apiCall('PATCH', `/api/trips/${tripId}`, carrier, step10Body);
  const podRequired = step10.status === 400 && (step10.data?.requiresPod || step10.data?.awaitingVerification);
  ...
  record({
    ...
    data: {
      ...
      note: podRequired ? 'Correctly blocked - POD required before completion' : undefined,
    },
    error: !podRequired ? 'Expected POD requirement but got different response' : undefined,
  });
  ...
  record({
    ...
    method: 'POST',
    body: { file: 'e2e-test-pod.png (1x1 PNG)', notes: 'E2E test POD document' },
    status: podUploadRes.status,
    pass: podUploadRes.status === 200 && !!podUploadData?.pod,
    data: podUploadData?.pod || podUploadData,
    error: podUploadRes.status !== 200 ? (podUploadData?.error || `Upload failed with status ${podUploadRes.status}`) : undefined,
  });
  ...
  record({
    ...
    error: step12.status !== 200 ? (step12.data?.error || `Verification failed with status ${step12.status}`) : undefined,
  });

  // ─── Step 13: Complete the trip (DELIVERED → COMPLETED) ─────────────────

  const step13Body = { status: 'COMPLETED' };
  const step13 = await apiCall('PATCH', `/api/trips/${tripId}`, carrier, step13Body);
  ...
  record({
    ...
    error: step13.status !== 200 ? (step13.data?.error || `Completion failed with status ${step13.status}`) : undefined,
  });
  ...
  record({
    ...
    pass: finalShipperWallet.status === 200 && finalCarrierWallet.status === 200,
    data: {
      shipper: {
        initialBalance: initialShipperBalance,
        finalBalance: finalShipperBalance,
        balanceChange: typeof finalShipperBalance === 'number' && typeof initialShipperBalance === 'number'
          ? finalShipperBalance - initialShipperBalance
          : 'N/A',
        wallets: finalShipperWallet.data?.wallets?.map((w: any) => ({ type: w.type, balance: w.balance })),
        recentTransactions: shipperTxns.data?.transactions?.slice(0, 3)?.map((t: any) => ({
          type: t.type,
          amount: t.amount,
      ...
      carrier: {
        ...
        balanceChange: typeof finalCarrierBalance === 'number' && typeof initialCarrierBalance === 'number'
          ? finalCarrierBalance - initialCarrierBalance
          : 'N/A',
        wallets: finalCarrierWallet.data?.wallets?.map((w: any) => ({ type: w.type, balance: w.balance })),
        recentTransactions: carrierTxns.data?.transactions?.slice(0, 3)?.map((t: any) => ({
          type: t.type,
          amount: t.amount,
          description: t.description?.slice(0, 60),
        })),
  ...
  record({
    ...
    error:
      loadStatus !== 'COMPLETED' ? `Load status is ${loadStatus}, expected COMPLETED` :
      tripStatus !== 'COMPLETED' ? `Trip status is ${tripStatus}, expected COMPLETED` :
      undefined,
  });
