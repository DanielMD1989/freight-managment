...
const rejected = await prisma.truck.count({ where: { carrierId: user.organizationId, approvalStatus: "REJECTED" } });
console.log("DB:", { totalTrucks, approved, pending, rejected });
...
console.log("Navigating to (carrier)/trucks...");
await page.goto("http://localhost:8088/(carrier)/trucks", { waitUntil: "domcontentloaded" });
