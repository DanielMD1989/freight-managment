...
const postLoadScreen = readFile(
  "mobile/lib/features/shipper/screens/post_load_screen.dart"
);
if (postLoadScreen) {
  const hasPriceField = /TextFormField.*(?:price|rate|fare|cost)/i.test(
    postLoadScreen
  );
  ...
  addResult({
    ...
    details: hasPriceField
      ? "Found price-related TextFormField"
...
const postTruckScreen = readFile(
  "mobile/lib/features/carrier/screens/carrier_post_trucks_screen.dart"
);
if (postTruckScreen) {
  ...
  addResult({
    ...
    details: hasExpectedRate
      ? "Found expectedRate references"
      : "No price fields",
...
if (loadRequestModal) {
  const hasProposedRateInput = /<input.*proposedRate|name="proposedRate"/i.test(
    loadRequestModal
  );
  const hasCommentNoPricing = /price negotiation happens outside/i.test(
    loadRequestModal
  );
  ...
  addResult({
    ...
    details: hasCommentNoPricing
      ? 'Has explicit "price negotiation outside" comment'
      : "No proposedRate input found",
    file: "app/carrier/loadboard/LoadRequestModal.tsx",
  });
}

// Check 1.4: Web Truck Booking Modal - NO offeredRate input
const truckBookingModal = readFile(
  "app/shipper/loadboard/TruckBookingModal.tsx"
);
if (truckBookingModal) {
  const hasOfferedRateInput = /<input.*offeredRate|name="offeredRate"/i.test(
    truckBookingModal
  );
  const hasCommentNoPricing = /price negotiation happens outside/i.test(
    truckBookingModal
...
const carrierLoadboard = readFile(
...
if (carrierLoadboard) {
  ...
  addResult({
    ...
    details: hasServiceFee
      ? "Service fee display found"
      : "No service fee display",
...
const shipperTruckboard = readFile(
  "mobile/lib/features/shipper/screens/shipper_truckboard_screen.dart"
);
if (shipperTruckboard) {
  const hasServiceFeeInfo = /service.*fee|calculated.*booking/i.test(
    shipperTruckboard
  );
  ...
  addResult({
    ...
    details: hasServiceFeeInfo
      ? "Service fee info found"
      : "No service fee info",
...
if (searchLoadsTab) {
  ...
  addResult({
    ...
    details: hasServiceFeeColumn
      ? "Service fee column found"
      : "No service fee column",
...
if (searchTrucksTab) {
  const hasServiceFeeColumn =
    /serviceFee|Service Fee|Calculated on booking/i.test(searchTrucksTab);
...
const mobileRequestScreens = [
  {
    path: "mobile/lib/features/carrier/screens/carrier_load_requests_screen.dart",
    name: "Carrier Load Requests",
  },
  {
    path: "mobile/lib/features/carrier/screens/carrier_truck_requests_screen.dart",
    name: "Carrier Truck Requests",
  },
  {
    path: "mobile/lib/features/shipper/screens/shipper_load_requests_screen.dart",
    name: "Shipper Load Requests",
  },
  {
    path: "mobile/lib/features/shipper/screens/shipper_truck_requests_screen.dart",
    name: "Shipper Truck Requests",
  },
];
...
for (const screen of mobileRequestScreens) {
  ...
  if (content) {
    const hasContactBox =
      /ContactToNegotiate|Contact.*Negotiate|url_launcher|launchUrl.*tel:/i.test(
        content
      );
...
const webRequestClients = [
  {
    path: "app/carrier/requests/MyLoadRequestsClient.tsx",
    name: "Carrier My Load Requests",
  },
  {
    path: "app/carrier/requests/ShipperRequestsClient.tsx",
    name: "Carrier Shipper Requests",
  },
  {
    path: "app/shipper/requests/LoadRequestsClient.tsx",
    name: "Shipper Load Requests",
  },
  {
    path: "app/shipper/requests/TruckRequestsClient.tsx",
    name: "Shipper Truck Requests",
  },
];
...
for (const client of webRequestClients) {
  ...
  if (content) {
    ...
    const hasApprovedCheck =
      /status.*===.*'APPROVED'|status.*===.*"APPROVED"/i.test(content);

    addResult({
      name: `Web ${client.name}: Contact to negotiate`,
      status: hasContactBox && hasApprovedCheck ? "PASS" : "FAIL",
      details: hasContactBox
        ? "Contact box with APPROVED check found"
        : "No contact box",
      file: client.path,
    });
  }
...
if (carrierLoadboard) {
  const hasVerifiedBadge = /isVerified|verified.*badge|✓.*Verified/i.test(
    carrierLoadboard
  );
...
if (shipperTruckboard) {
  const hasVerifiedBadge =
    /isVerified|verified.*badge|✓.*Verified|carrierIsVerified/i.test(
      shipperTruckboard
    );
...
addResult({
  ...
  status: checkFileExists("mobile/lib/core/services/gps_service.dart")
    ? "PASS"
    : "FAIL",
...
addResult({
  ...
  status: checkFileExists("app/api/corridors/calculate-fee/route.ts")
    ? "PASS"
    : "FAIL",
...
if (prismaSchema) {
  ...
  addResult({
    ...
    status:
      hasCorridorModel && hasDistanceKm && hasPricePerKm ? "PASS" : "FAIL",
