...
const user = await prisma.user.findUnique({ where: { email: "carrier@test.com" }, select: { organizationId: true } });
const totalTrips = await prisma.trip.count({ where: { carrierId: user.organizationId } });
const byStatus = {};
for (const s of ["ASSIGNED","PICKUP_PENDING","IN_TRANSIT","DELIVERED","COMPLETED","EXCEPTION","CANCELLED"]) {
  byStatus[s] = await prisma.trip.count({ where: { carrierId: user.organizationId, status: s } });
}
console.log("DB total carrier trips:", totalTrips);
console.log("DB by status:", byStatus);
...
console.log("Navigating to (carrier)/trips...");
await page.goto("http://localhost:8088/(carrier)/trips", { waitUntil: "domcontentloaded" });
