...
async function createTestUsers() {
  try {
    ...
    let shipperOrg = await db.organization.findFirst({
      where: { name: "Test Shipper" },
    });
    if (!shipperOrg) {
    ...
    let shipperUser = await db.user.findUnique({
    ...
    if (!shipperUser) {
      ...
      console.log(
        "✅ Shipper exists, status set to ACTIVE: shipper@test.com / Test123!"
      );
    }

    // Create carrier org & user
    let carrierOrg = await db.organization.findFirst({
    ...
    let carrierUser = await db.user.findUnique({
    ...
    if (!carrierUser) {
      ...
      console.log(
        "✅ Carrier exists, status set to ACTIVE: carrier@test.com / Test123!"
      );
    }
