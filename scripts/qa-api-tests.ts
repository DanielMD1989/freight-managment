...
async function testAuth() {
  ...
  await runTest("1.2.4 Rate limit after 5 failed attempts", async () => {
    ...
    assert(
      lastStatus === 429,
      `Expected 429 after rate limit, got ${lastStatus}`
    );
  });
}
...
async function testAPIProtection() {
  await runTest(
    "2.2.1 Unauthenticated access to /api/loads returns 401",
    async () => {
  ...
  await runTest(
    "6.1.1 CSRF protection on state-changing endpoints",
    async () => {
...
async function testSecurityHeaders() {
  ...
  await runTest("API rejects invalid content types gracefully", async () => {
    ...
    assert(
      ...
      `Expected 400/415, got ${res.status}`
    );
  });
}
...
async function testWebSocketEndpoint() {
  await runTest("WebSocket endpoint path configured", async () => {
    ...
    assert(
      res.status !== 404,
      `Socket endpoint should exist, got ${res.status}`
    );
  });
}
