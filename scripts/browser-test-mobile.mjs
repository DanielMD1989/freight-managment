...
async function main() {
  ...
  try {
    ...
    if (bodyText.includes("Find Loads") || bodyText.includes("Skip")) {
      ...
      await sleep(2000);
      await page.screenshot({
        path: `${DIR}/mobile-02-after-onboarding.png`,
        fullPage: true,
      });
    ...
    for (let i = 0; i < allInputs.length; i++) {
      ...
      if (
        attrs.type === "email" ||
        attrs.placeholder?.toLowerCase().includes("email") ||
        attrs.autoComplete === "email" ||
        attrs["aria-label"]?.toLowerCase().includes("email")
      ) {
        emailInput = allInputs[i];
      } else if (
        attrs.type === "password" ||
        attrs.placeholder?.toLowerCase().includes("password") ||
        attrs.autoComplete === "current-password" ||
        attrs["aria-label"]?.toLowerCase().includes("password")
      ) {
        pwInput = allInputs[i];
      }
    }
    ...
    await page.screenshot({
      path: `${DIR}/mobile-03-login-filled.png`,
      fullPage: true,
    });
    ...
    if (emailInput || pwInput) {
      ...
      await sleep(5000);
      await page.screenshot({
        path: `${DIR}/mobile-04-after-login.png`,
        fullPage: true,
      });
    ...
    const hasDashboard =
      ...
      dashText.includes("Active") ||
      dashText.includes("Total Loads") ||
      dashText.includes("In Transit");
    console.log(`   Has dashboard content: ${hasDashboard}`);
    if (hasDashboard) {
      console.log(`   Dashboard text: ${dashText.substring(0, 400)}`);
    }
    await page.screenshot({
      path: `${DIR}/mobile-05-dashboard.png`,
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: `${DIR}/mobile-08-shipments.png`,
      fullPage: true,
    });
  } catch (e) {
    console.log(`\nError: ${e.message}`);
    await page
      .screenshot({ path: `${DIR}/mobile-error.png`, fullPage: true })
      .catch(() => {});
  }
