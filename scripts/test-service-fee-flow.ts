...
async function main() {
  ...
  if (corridor) {
    ...
    console.log(`   Base Fee: ${distanceKm} km × ${pricePerKm} ETB = ${baseFee.toFixed(2)} ETB`);
    if (discount > 0) {
      console.log(`   Promo Discount: -${discount.toFixed(2)} ETB (${corridor.promoDiscountPct}%)`);
    }
  ...
  if (corridor && platformAccount && shipperWallets.length > 0) {
    ...
    const serviceFee = Number(corridor.distanceKm) * Number(corridor.pricePerKm);
    ...
    console.log(`   3. Fee reserved from shipper wallet: ${testShipperWallet.organization?.name}`);
    ...
    if (loadWithCorridor) {
      console.log(`Found load with corridor: ${loadWithCorridor.id.slice(0, 8)}...`);
      ...
      console.log('(Loads will be linked when assigned through the corridor system)');
    }
  } else {
