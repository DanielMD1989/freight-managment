...
async function testWebShipper(browser) {
  ...
  try {
    ...
    // ── 2. DASHBOARD ──────────────────────────────────────────────────
    console.log("\n[WEB] 2. Dashboard");
    await page.goto(`${WEB_URL}/shipper/dashboard`, {
    ...
    r.dashboardRecentLoads = hasText(
      dashText,
      "Recent Activity",
      "Recent Loads",
      "Recent"
    ...
    const postNewLoadClicked = await tryClick(
      page,
      ['a:has-text("Post New Load")', 'a[href="/shipper/loads/create"]'],
      "Post New Load"
    );
    if (postNewLoadClicked) {
      ...
      console.log(
        `   Navigated to create: ${r.dashboardPostNewLoad ? "✓" : "✗"}`
      );
    ...
    const viewAllClicked = await tryClick(
      page,
      [
    ...
    // ── 3. MY LOADS ───────────────────────────────────────────────────
    console.log("\n[WEB] 3. My Loads");
    await page.goto(`${WEB_URL}/shipper/loads`, {
      waitUntil: "domcontentloaded",
    });
    await sleep(3000);
    ...
    for (const status of webStatusFilters) {
      ...
      await page.goto(`${WEB_URL}/shipper/loads?status=${status}`, {
        waitUntil: "domcontentloaded",
      });
      await sleep(2000);
      const filterLinks = await page.$$(
        'a[href*="/shipper/loads/c"]:not([href*="create"])'
      );
    ...
    await sleep(2000);
    const firstLoadLink = await page.$(
      'a[href*="/shipper/loads/c"]:not([href*="create"])'
    );
    if (firstLoadLink) {
      ...
      let editClicked = await tryClick(
        page,
      ...
      if (!editClicked) {
        ...
        await page.goto(`${WEB_URL}/shipper/loads?status=posted`, {
          waitUntil: "domcontentloaded",
        });
        await sleep(2000);
        const postedLink = await page.$(
          'a[href*="/shipper/loads/c"]:not([href*="create"])'
        );
        if (postedLink) {
          await postedLink.click();
          await sleep(3000);
          editClicked = await tryClick(
            page,
            [
              'a:has-text("Edit")',
    ...
    await page.goto(`${WEB_URL}/shipper/loads/create`, {
      waitUntil: "domcontentloaded",
    });
    await sleep(2500);
    ...
    try {
      ...
      const pickupCityInput = await page.$(
        'select[name="pickupCity"], input[name="pickupCity"], [id="pickupCity"]'
      );
      if (pickupCityInput) {
        const tagName = await pickupCityInput.evaluate((el) =>
      ...
      const deliveryCityInput = await page.$(
        'select[name="deliveryCity"], input[name="deliveryCity"], [id="deliveryCity"]'
      );
      if (deliveryCityInput) {
        const tagName = await deliveryCityInput.evaluate((el) =>
          el.tagName.toLowerCase()
        );
        if (tagName === "select") {
      ...
      const pickupDateInput = await page.$(
        'input[name="pickupDate"], input[type="date"]:first-of-type'
      );
      if (pickupDateInput) {
      ...
      const deliveryDateInput = await page.$(
        'input[name="deliveryDate"], input[type="date"]:nth-of-type(2)'
      );
    ...
    const step2Clicked = await tryClick(
      page,
    ...
    if (step2Clicked) {
      ...
      const truckChipClicked = await tryClick(
        page,
        [
      ...
      const weightInput = await page.$(
        'input[name="weight"], input[placeholder*="weight" i], input[type="number"]'
      );
      if (weightInput) {
      ...
      const cargoInput = await page.$(
        'textarea[name="cargoDescription"], input[name="cargoDescription"], textarea'
      );
      if (cargoInput) {
        await cargoInput.fill(
          "Agricultural products - coffee beans for export"
        );
      ...
      const step3Clicked = await tryClick(
        page,
        ['button:has-text("Next")', 'button:has-text("Continue")'],
        "Next → Step 3"
      );
      if (step3Clicked) {
        ...
        const step4Clicked = await tryClick(
          page,
          ['button:has-text("Next")', 'button:has-text("Continue")'],
          "Next → Step 4 Review"
        );
        if (step4Clicked) {
          ...
          r.createStep4Review =
            hasText(reviewText, "Addis Ababa", "Review") ||
            hasText(reviewText, "FLATBED", "Cargo", "Summary");
          console.log(
            `   Review shows data: ${r.createStep4Review ? "✓" : "✗"}`
          );
    ...
    // ── 6. LOADBOARD ──────────────────────────────────────────────────
    console.log("\n[WEB] 6. Loadboard");
    await page.goto(`${WEB_URL}/shipper/loadboard`, {
      waitUntil: "domcontentloaded",
    });
    await sleep(3000);
    ...
    const searchTabClicked = await tryClick(
      page,
      [
    ...
    if (searchTabClicked) {
      ...
      r.loadboardSearchTrucks = hasText(
        searchText,
        "Search",
        "Truck",
        "Origin",
      ...
      console.log(
        `   Search trucks tab: ${r.loadboardSearchTrucks ? "✓" : "✗"}`
      );
      await screenshot(page, "web-08b-loadboard-search-trucks");

      // Try applying a filter
      const truckTypeFilter = await page.$(
        'select[name="truckType"], select:has(option[value="FLATBED"])'
      );
      if (truckTypeFilter) {
    ...
    // ── 7. REQUESTS ───────────────────────────────────────────────────
    console.log("\n[WEB] 7. Requests");
    await page.goto(`${WEB_URL}/shipper/requests`, {
      waitUntil: "domcontentloaded",
    });
    await sleep(3000);
    ...
    const truckReqTabClicked = await tryClick(
      page,
      [
    ...
    const loadReqTabClicked = await tryClick(
      page,
      [
    ...
    if (loadReqTabClicked) {
      ...
      console.log(
        `   Approve/Reject buttons: ${r.requestsActions ? "✓ found" : "✗ none visible"}`
      );
    }

    // ── 8. TRIPS ──────────────────────────────────────────────────────
    console.log("\n[WEB] 8. Trips");
    await page.goto(`${WEB_URL}/shipper/trips`, {
      waitUntil: "domcontentloaded",
    });
    await sleep(3000);
    ...
    // ── 9. WALLET ─────────────────────────────────────────────────────
    console.log("\n[WEB] 9. Wallet");
    await page.goto(`${WEB_URL}/shipper/wallet`, {
      waitUntil: "domcontentloaded",
    });
    await sleep(3000);
    ...
    // ── 10. MAP ───────────────────────────────────────────────────────
    console.log("\n[WEB] 10. Map");
    await page.goto(`${WEB_URL}/shipper/map`, {
      waitUntil: "domcontentloaded",
    });
    await sleep(3000);
    ...
    // ── 11. SETTINGS / PROFILE ────────────────────────────────────────
    console.log("\n[WEB] 11. Settings");
    await page.goto(`${WEB_URL}/shipper/settings`, {
      waitUntil: "domcontentloaded",
    });
    await sleep(3000);
    ...
    // ── 12. DOCUMENTS ─────────────────────────────────────────────────
    console.log("\n[WEB] 12. Documents");
    await page.goto(`${WEB_URL}/shipper/documents`, {
      waitUntil: "domcontentloaded",
    });
    await sleep(2500);
    ...
    // ── 13. TEAM ──────────────────────────────────────────────────────
    console.log("\n[WEB] 13. Team");
    await page.goto(`${WEB_URL}/shipper/team`, {
      waitUntil: "domcontentloaded",
    });
    await sleep(2500);
    ...
    // ── 14. MATCHES ───────────────────────────────────────────────────
    console.log("\n[WEB] 14. Matches");
    await page.goto(`${WEB_URL}/shipper/matches`, {
      waitUntil: "domcontentloaded",
    });
    await sleep(2500);
  ...
  try {
    ...
    if (recentLoad) {
      ...
      r.dashboardRecentLoadClick =
        page.url().includes("loads/") ||
        (await safeText(page)).includes("Cargo");
      console.log(
    ...
    const loadsTabClicked = await tryClick(
      page,
      [
    ...
    try {
      ...
      if (mStep2) {
        ...
        if (mStep3) {
          ...
          // Next → Step 4 Review
          const mStep4 = await clickByExactText(
            page,
            "Next",
            "Next → Step 4 Review"
          );
    ...
    try {
      const mTrucksTabClicked = await navigateToTab(
        page,
        "/trucks",
        "Find Trucks"
      );
    ...
    try {
      ...
      const mapTabClicked = await tryClick(
        page,
        [
          'a[role="tab"][href="/map"]',
          'a[href="/map"]',
...
function printParityTable() {
  ...
  const features = [
    { name: "Login & redirect", web: web.login, mobile: mob.login },
    {
      ...
      web: web.dashboardStats,
      mobile: mob.dashboardStats,
    },
    {
      name: "Dashboard quick actions",
      web: web.dashboardQuickActions,
      mobile: mob.dashboardQuickActions,
    },
    {
      name: "Dashboard recent loads",
      web: web.dashboardRecentLoads,
      mobile: mob.dashboardRecentLoads,
    },
    { name: "Dashboard spending", web: web.dashboardSpending, mobile: false },
    {
      name: "Dashboard loads by status",
      web: web.dashboardLoadsByStatus,
      mobile: mob.dashboardLoadsByStatus,
    },
    { name: "Load list + filters", web: web.loadsList, mobile: mob.loadsList },
    {
      name: "Load detail",
      web: web.loadDetailRoute,
      mobile: mob.loadDetailRoute,
    },
    {
      name: "Load detail status",
      web: web.loadDetailStatus,
      mobile: mob.loadDetailStatus,
    },
    {
      name: "Load detail cargo",
      web: web.loadDetailCargo,
      mobile: mob.loadDetailCargo,
    },
    {
      name: "Load detail actions",
      web: web.loadDetailActions,
      mobile: mob.loadDetailActions,
    },
    { name: "Edit load", web: web.loadEdit, mobile: mob.loadEdit },
    {
      name: "Create load wizard",
      web: web.createLoadWizard,
      mobile: mob.createLoadWizard,
    },
    {
      name: "Create step 1 (Route)",
      web: web.createStep1,
      mobile: mob.createStep1,
    },
    {
      name: "Create step 2 (Cargo)",
      web: web.createStep2,
      mobile: mob.createStep2,
    },
    {
      name: "Create step 3 (Options)",
      web: web.createStep3,
      mobile: mob.createStep3,
    },
    {
      name: "Create step 4 (Review)",
      web: web.createStep4Review,
      mobile: mob.createStep4Review,
    },
    {
      name: "Loadboard / Find trucks",
      web: web.loadboardPage,
      mobile: mob.findTrucksPage,
    },
    {
      name: "Truck search + filters",
      web: web.loadboardSearchTrucks,
      mobile: mob.findTrucksSearch,
    },
  ...
  for (const f of features) {
    ...
    const match =
      f.web === false && f.mobile === false
        ? "—"
        : !!f.web === !!f.mobile
          ? "✓"
          : "✗";
    console.log(
      `| ${f.name.padEnd(27)} | ${w.padEnd(3)} | ${m.padEnd(6)} | ${match.padEnd(6)} |`
    );
  }
...
async function main() {
  ...
  console.log(
    "║  Comprehensive Shipper Browser Test — Web vs Mobile             ║"
  );
