...
export default function RegisterPage() {
      ...
      if (!response.ok) {
        if (data.details && Array.isArray(data.details)) {
          const errorMessages = data.details
            .map((detail: ValidationErrorDetail) => detail.message)
            .join(", ");
  ...
  const inputClassName =
    "block w-full rounded-xl border-0 px-4 py-3 text-slate-900 dark:text-white bg-white dark:bg-slate-900 ring-1 ring-inset ring-slate-300 dark:ring-slate-700 placeholder:text-slate-400 focus:ring-2 focus:ring-inset focus:ring-primary-600 dark:focus:ring-primary-500 transition-all text-sm";
  const labelClassName =
    "block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2";
  const selectClassName =
    "block w-full rounded-xl border-0 px-4 py-3 text-slate-900 dark:text-white bg-white dark:bg-slate-900 ring-1 ring-inset ring-slate-300 dark:ring-slate-700 focus:ring-2 focus:ring-inset focus:ring-primary-600 dark:focus:ring-primary-500 transition-all text-sm";

  return (
    <div className="flex min-h-screen">
      {/* Left Panel - Brand */}
      <div className="via-primary-900 relative hidden overflow-hidden bg-gradient-to-br from-slate-900 to-slate-900 lg:flex lg:w-5/12">
        {/* Decorative Elements */}
        ...
        <div className="relative z-10 flex flex-col justify-center px-12">
          ...
          <h1 className="mb-6 text-3xl leading-tight font-bold text-white">
            Join Ethiopia&apos;s
            <br />
            <span className="text-primary-400">Largest Freight Network</span>
          </h1>

          <p className="mb-8 max-w-sm text-base text-slate-300">
            Whether you&apos;re shipping goods or hauling freight, we&apos;ve
            got you covered.
          </p>
          ...
          <div className="space-y-4">
            {[
              {
                title: "For Shippers",
                desc: "Find reliable carriers instantly",
              },
              {
                title: "For Carriers",
                desc: "Get matched with profitable loads",
              },
              { title: "For All", desc: "Track shipments in real-time" },
            ].map((item, i) => (
              <div key={i} className="flex items-start gap-3">
                <div className="bg-accent-500/30 mt-0.5 flex h-6 w-6 items-center justify-center rounded-full">
                  <svg
                    className="text-accent-400 h-4 w-4"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
      ...
      <div className="flex w-full items-center justify-center overflow-y-auto bg-slate-50 px-6 py-8 lg:w-7/12 dark:bg-slate-950">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            {formData.role === "CARRIER" && (
              <>
                <div>
                  ...
                  >
                    <option value="CARRIER_COMPANY">
                      Trucking Company (multiple trucks)
                    </option>
                    <option value="CARRIER_INDIVIDUAL">
                      Single Truck Owner
                    </option>
                    <option value="FLEET_OWNER">
                      Fleet Owner (independent)
                    </option>
                  </select>
                  <p className="mt-1.5 text-xs text-slate-500 dark:text-slate-400">
                    {formData.carrierType === "CARRIER_COMPANY" &&
                      "A trucking company with multiple trucks under management"}
                    {formData.carrierType === "CARRIER_INDIVIDUAL" &&
                      "An owner-operator with a single truck, may join an association"}
                    {formData.carrierType === "FLEET_OWNER" &&
                      "An independent fleet operator with multiple owned trucks"}
                  </p>
                </div>
                ...
                {formData.carrierType === "CARRIER_INDIVIDUAL" && (
                  <div>
                    <label htmlFor="associationId" className={labelClassName}>
