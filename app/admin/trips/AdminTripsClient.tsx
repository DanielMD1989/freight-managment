...
type TripStatus = 'ALL' | 'ASSIGNED' | 'PICKUP_PENDING' | 'IN_TRANSIT' | 'DELIVERED' | 'COMPLETED' | 'CANCELLED';
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/60 dark:border-slate-700 shadow-sm p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-slate-800 dark:text-white">{summary.totalTrips.toLocaleString()}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/60 dark:border-slate-700 shadow-sm p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-slate-500 dark:text-slate-400">Total Service Fees</p>
              <p className="text-2xl font-bold text-slate-800 dark:text-white">{formatCurrency(summary.totalServiceFees)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/60 dark:border-slate-700 shadow-sm p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-slate-800 dark:text-white">{summary.activeTrips.toLocaleString()}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-700">
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Trip ID</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Load Route</th>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Truck</th>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Service Fee</th>
                ...
                trips.map((trip) => (
                  <tr key={trip.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-700 dark:text-slate-300">
                      {trip.load ? `${trip.load.pickupCity} → ${trip.load.deliveryCity}` : '-'}
                    </td>
                    <td className="px-4 py-3 text-sm text-slate-700 dark:text-slate-300">
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${STATUS_COLORS[trip.status] || 'bg-gray-100 text-gray-800'}`}>
