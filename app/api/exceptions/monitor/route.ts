  ...
  try {
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        loadsWithIssues: summary.filter((s): s is { loadId: string; route: string; created: number; rules: string[] } =>
      ...
      loads: summary,
      stats: {
        last24Hours: exceptionStats.reduce((acc, s) => {
  ...
  try {
    ...
    return NextResponse.json({
      monitoringConfig: {
        ...
        rules: [
          { name: 'Late Pickup', threshold: '2 hours past scheduled time', priority: 'MEDIUM-HIGH' },
          { name: 'Late Delivery', threshold: '2 hours past scheduled time', priority: 'HIGH-CRITICAL' },
          { name: 'GPS Offline', threshold: '4 hours without signal', priority: 'HIGH-CRITICAL' },
          { name: 'Stalled Load', threshold: '<1km movement in 4 hours', priority: 'CRITICAL' },
        ],
      },
      recentEscalations: recentAutoEscalations,
      stats: {
        last24Hours: recentAutoEscalations.length,
        byType: escalationStats.reduce((acc, s) => {
