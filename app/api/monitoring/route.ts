...
import { getMonitoringData, getMonitoringSummary, getAllAlerts, resolveAlert } from '@/lib/monitoring';
import { logger } from '@/lib/logger';

/**
 * Verify admin access or monitoring API key
 */
async function verifyAccess(request: NextRequest): Promise<{ authorized: boolean; userId?: string }> {
  // Check for monitoring API key (for external monitoring tools)
  const apiKey = request.headers.get('x-monitoring-api-key');
  if (apiKey && process.env.MONITORING_API_KEY && apiKey === process.env.MONITORING_API_KEY) {
