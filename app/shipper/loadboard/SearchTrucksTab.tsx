...
export default function SearchTrucksTab({
  ...
  const [selectedTruckPosting, setSelectedTruckPosting] =
    useState<TruckPostingWithCarrier | null>(null);
  const [editingSearch, setEditingSearch] = useState<SavedSearch | null>(null);
  const [pendingRequestTruckIds, setPendingRequestTruckIds] = useState<
    Set<string>
  >(new Set());
  ...
  const fetchTrucks = async (overrideFilters?: FilterValues) => {
    ...
    try {
      ...
      if (
        ...
        currentFilters.destination.trim() !== "" &&
        currentFilters.destination.toLowerCase() !== "anywhere"
      ) {
    ...
    return [
      // L27 FIX: Properly typed column render functions
      {
        ...
        render: (_: unknown, row: TruckPostingWithCarrier) => (
          <AgeIndicator date={row.createdAt} />
        ),
      },
      {
        key: "availableDate",
    ...
    () => [
      {
        ...
        show: (row: TruckPostingWithCarrier) =>
          !pendingRequestTruckIds.has(row.truck?.id || row.truckId),
      },
      {
        key: "pending",
        label: "Pending",
        ...
        onClick: () => {
          alert(
            "Your booking request is pending carrier approval. You will be notified when they respond."
          );
        },
        show: (row: TruckPostingWithCarrier) =>
          pendingRequestTruckIds.has(row.truck?.id || row.truckId),
    ...
    <div className="space-y-6">
        ...
        >
          ...
          >
            <path
              ...
              strokeLinejoin="round"
              strokeWidth={2}
              d={showSearchForm ? "M5 15l7-7 7 7" : "M19 9l-7 7-7-7"}
            />
          </svg>
      ...
      {showSearchForm && (
        ...
        >
          ...
          {/* Search Row */}
          <div
            className="grid grid-cols-10 items-center gap-2 px-4 py-4 text-xs"
        ...
        >
          <div>
            ...
            >
              {loading
                ? "Searching..."
                : "Available trucks matching your criteria"}
            </p>
          </div>
          <StatusTabs
            tabs={resultsTabs}
