...
describe("lib/loadStateMachine", () => {
  ...
  describe("isValidTransition", () => {
    describe("DRAFT transitions", () => {
      it("should allow DRAFT → POSTED", () => {
        expect(isValidTransition(LoadStatus.DRAFT, LoadStatus.POSTED)).toBe(
          true
        );
      });

      it("should allow DRAFT → CANCELLED", () => {
        expect(isValidTransition(LoadStatus.DRAFT, LoadStatus.CANCELLED)).toBe(
          true
        );
      });

      it("should NOT allow DRAFT → IN_TRANSIT", () => {
        expect(isValidTransition(LoadStatus.DRAFT, LoadStatus.IN_TRANSIT)).toBe(
          false
        );
      });
    });
    ...
    describe("POSTED transitions", () => {
      it("should allow POSTED → SEARCHING", () => {
        expect(isValidTransition(LoadStatus.POSTED, LoadStatus.SEARCHING)).toBe(
          true
        );
      });

      it("should allow POSTED → OFFERED", () => {
        expect(isValidTransition(LoadStatus.POSTED, LoadStatus.OFFERED)).toBe(
          true
        );
      });

      it("should allow POSTED → ASSIGNED", () => {
        expect(isValidTransition(LoadStatus.POSTED, LoadStatus.ASSIGNED)).toBe(
          true
        );
      });

      it("should allow POSTED → UNPOSTED", () => {
        expect(isValidTransition(LoadStatus.POSTED, LoadStatus.UNPOSTED)).toBe(
          true
        );
      });

      it("should allow POSTED → CANCELLED", () => {
        expect(isValidTransition(LoadStatus.POSTED, LoadStatus.CANCELLED)).toBe(
          true
        );
      });

      it("should allow POSTED → EXPIRED", () => {
        expect(isValidTransition(LoadStatus.POSTED, LoadStatus.EXPIRED)).toBe(
          true
        );
      });
    });
    ...
    describe("IN_TRANSIT transitions", () => {
      it("should allow IN_TRANSIT → DELIVERED", () => {
        expect(
          isValidTransition(LoadStatus.IN_TRANSIT, LoadStatus.DELIVERED)
        ).toBe(true);
      });

      it("should allow IN_TRANSIT → EXCEPTION", () => {
        expect(
          isValidTransition(LoadStatus.IN_TRANSIT, LoadStatus.EXCEPTION)
        ).toBe(true);
      });

      it("should NOT allow IN_TRANSIT → CANCELLED directly (business decision)", () => {
        expect(
          isValidTransition(LoadStatus.IN_TRANSIT, LoadStatus.CANCELLED)
        ).toBe(false);
      });
    });
    ...
    describe("terminal states", () => {
      it("CANCELLED should have no valid transitions", () => {
        expect(isValidTransition(LoadStatus.CANCELLED, LoadStatus.POSTED)).toBe(
          false
        );
        expect(isValidTransition(LoadStatus.CANCELLED, LoadStatus.DRAFT)).toBe(
          false
        );
      });
    ...
    describe("EXCEPTION recovery", () => {
      it("should allow EXCEPTION → SEARCHING", () => {
        expect(
          isValidTransition(LoadStatus.EXCEPTION, LoadStatus.SEARCHING)
        ).toBe(true);
      });

      it("should allow EXCEPTION → ASSIGNED", () => {
        expect(
          isValidTransition(LoadStatus.EXCEPTION, LoadStatus.ASSIGNED)
        ).toBe(true);
      });

      it("should allow EXCEPTION → CANCELLED", () => {
        expect(
          isValidTransition(LoadStatus.EXCEPTION, LoadStatus.CANCELLED)
        ).toBe(true);
      });

      it("should allow EXCEPTION → COMPLETED", () => {
        expect(
          isValidTransition(LoadStatus.EXCEPTION, LoadStatus.COMPLETED)
        ).toBe(true);
      });
    });
    ...
    describe("EXPIRED/UNPOSTED recovery", () => {
      it("should allow EXPIRED → POSTED", () => {
        expect(isValidTransition(LoadStatus.EXPIRED, LoadStatus.POSTED)).toBe(
          true
        );
      });

      it("should allow UNPOSTED → POSTED", () => {
        expect(isValidTransition(LoadStatus.UNPOSTED, LoadStatus.POSTED)).toBe(
          true
        );
      });
    });
  ...
  describe("canRoleSetStatus", () => {
    ...
    describe("CARRIER permissions", () => {
      ...
      it("CARRIER can set PICKUP_PENDING", () => {
        expect(canRoleSetStatus("CARRIER", LoadStatus.PICKUP_PENDING)).toBe(
          true
        );
      });
    ...
    describe("DISPATCHER permissions", () => {
      ...
      it("DISPATCHER cannot set DELIVERED", () => {
        expect(canRoleSetStatus("DISPATCHER", LoadStatus.DELIVERED)).toBe(
          false
        );
      });
    });
  ...
  describe("getStatusDescription", () => {
    it("should return description for DRAFT", () => {
      expect(getStatusDescription(LoadStatus.DRAFT)).toContain(
        "not yet posted"
      );
    });

    it("should return description for IN_TRANSIT", () => {
      expect(getStatusDescription(LoadStatus.IN_TRANSIT)).toContain(
        "in transit"
      );
    });
    ...
    it("should return unknown for invalid status", () => {
      expect(getStatusDescription("INVALID" as LoadStatus)).toBe(
        "Unknown status"
      );
    });
  });
  ...
  describe("complete workflow scenarios", () => {
    it("should support happy path: DRAFT → POSTED → ASSIGNED → IN_TRANSIT → DELIVERED → COMPLETED", () => {
      ...
      expect(
        isValidTransition(LoadStatus.ASSIGNED, LoadStatus.IN_TRANSIT)
      ).toBe(true);
      expect(
        isValidTransition(LoadStatus.IN_TRANSIT, LoadStatus.DELIVERED)
      ).toBe(true);
      expect(
        isValidTransition(LoadStatus.DELIVERED, LoadStatus.COMPLETED)
      ).toBe(true);
    });
