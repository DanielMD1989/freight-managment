...
describe('lib/tripStateMachine', () => {
  ...
  describe('isValidTripTransition', () => {
    describe('ASSIGNED transitions', () => {
      it('should allow ASSIGNED → PICKUP_PENDING', () => {
        expect(isValidTripTransition(TripStatus.ASSIGNED, TripStatus.PICKUP_PENDING)).toBe(true);
      });
      ...
      it('should NOT allow ASSIGNED → IN_TRANSIT directly', () => {
        expect(isValidTripTransition(TripStatus.ASSIGNED, TripStatus.IN_TRANSIT)).toBe(false);
      });
    });

    describe('PICKUP_PENDING transitions', () => {
      it('should allow PICKUP_PENDING → IN_TRANSIT', () => {
        expect(isValidTripTransition(TripStatus.PICKUP_PENDING, TripStatus.IN_TRANSIT)).toBe(true);
      });

      it('should allow PICKUP_PENDING → CANCELLED', () => {
        expect(isValidTripTransition(TripStatus.PICKUP_PENDING, TripStatus.CANCELLED)).toBe(true);
      });
    });

    describe('IN_TRANSIT transitions', () => {
      it('should allow IN_TRANSIT → DELIVERED', () => {
        expect(isValidTripTransition(TripStatus.IN_TRANSIT, TripStatus.DELIVERED)).toBe(true);
      });

      it('should allow IN_TRANSIT → CANCELLED', () => {
        expect(isValidTripTransition(TripStatus.IN_TRANSIT, TripStatus.CANCELLED)).toBe(true);
      });
    });

    describe('DELIVERED transitions', () => {
      it('should allow DELIVERED → COMPLETED', () => {
        expect(isValidTripTransition(TripStatus.DELIVERED, TripStatus.COMPLETED)).toBe(true);
      });

      it('should allow DELIVERED → CANCELLED', () => {
        expect(isValidTripTransition(TripStatus.DELIVERED, TripStatus.CANCELLED)).toBe(true);
      });
    });

    describe('terminal states', () => {
      it('COMPLETED should have no valid transitions', () => {
        expect(isValidTripTransition(TripStatus.COMPLETED, TripStatus.CANCELLED)).toBe(false);
        expect(isValidTripTransition(TripStatus.COMPLETED, TripStatus.ASSIGNED)).toBe(false);
      });

      it('CANCELLED should have no valid transitions', () => {
        expect(isValidTripTransition(TripStatus.CANCELLED, TripStatus.COMPLETED)).toBe(false);
  ...
  describe('canRoleSetTripStatus', () => {
    describe('CARRIER permissions', () => {
      it('CARRIER can set PICKUP_PENDING', () => {
        expect(canRoleSetTripStatus('CARRIER', TripStatus.PICKUP_PENDING)).toBe(true);
      });

      it('CARRIER can set IN_TRANSIT', () => {
        expect(canRoleSetTripStatus('CARRIER', TripStatus.IN_TRANSIT)).toBe(true);
      });

      it('CARRIER can set DELIVERED', () => {
        expect(canRoleSetTripStatus('CARRIER', TripStatus.DELIVERED)).toBe(true);
      });

      it('CARRIER cannot set ASSIGNED', () => {
        expect(canRoleSetTripStatus('CARRIER', TripStatus.ASSIGNED)).toBe(false);
      });

      it('CARRIER cannot set CANCELLED', () => {
        expect(canRoleSetTripStatus('CARRIER', TripStatus.CANCELLED)).toBe(false);
      });
    });

    describe('DISPATCHER permissions', () => {
      it('DISPATCHER can set ASSIGNED', () => {
        expect(canRoleSetTripStatus('DISPATCHER', TripStatus.ASSIGNED)).toBe(true);
      });

      it('DISPATCHER can set CANCELLED', () => {
        expect(canRoleSetTripStatus('DISPATCHER', TripStatus.CANCELLED)).toBe(true);
      });

      it('DISPATCHER cannot set IN_TRANSIT', () => {
        expect(canRoleSetTripStatus('DISPATCHER', TripStatus.IN_TRANSIT)).toBe(false);
      });
    });
    ...
    describe('unknown role', () => {
      it('unknown role cannot set any status', () => {
        expect(canRoleSetTripStatus('UNKNOWN', TripStatus.ASSIGNED)).toBe(false);
        expect(canRoleSetTripStatus('SHIPPER', TripStatus.DELIVERED)).toBe(false);
      });
    });
  });
  ...
  describe('getTripStatusDescription', () => {
    it('should return description for ASSIGNED', () => {
      expect(getTripStatusDescription(TripStatus.ASSIGNED)).toContain('accepted');
    });

    it('should return description for IN_TRANSIT', () => {
      expect(getTripStatusDescription(TripStatus.IN_TRANSIT)).toContain('in transit');
    });

    it('should return description for COMPLETED', () => {
      expect(getTripStatusDescription(TripStatus.COMPLETED)).toContain('POD verified');
    });

    it('should return unknown for invalid status', () => {
      expect(getTripStatusDescription('INVALID' as TripStatus)).toBe('Unknown status');
    });
  });
  ...
  describe('complete workflow scenarios', () => {
    ...
    it('should not allow backward transitions', () => {
      expect(isValidTripTransition(TripStatus.IN_TRANSIT, TripStatus.PICKUP_PENDING)).toBe(false);
      expect(isValidTripTransition(TripStatus.DELIVERED, TripStatus.IN_TRANSIT)).toBe(false);
    });
  });
});
