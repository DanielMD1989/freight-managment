...
jest.mock('@/lib/serviceFeeCalculation', () => ({
  ...
  calculatePartyFee: jest.fn((distance, pricePerKm, promoFlag, promoPct) => {
    if (distance <= 0 || pricePerKm <= 0 || !Number.isFinite(distance) || !Number.isFinite(pricePerKm)) {
      return { baseFee: 0, finalFee: 0, promoDiscount: 0, promoApplied: false, pricePerKm: 0 };
    }
...
describe('lib/serviceFeeManagement', () => {
  ...
  describe('validateWalletBalancesForTrip', () => {
    ...
    it('should detect insufficient shipper balance', async () => {
      ...
      expect(result.errors[0]).toContain('Shipper has insufficient wallet balance');
    });
    ...
    it('should detect insufficient carrier balance', async () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.length).toBeGreaterThan(0);
      expect(result.errors[0]).toContain('Carrier has insufficient wallet balance');
    });
  ...
  describe('assignCorridorToLoad', () => {
    const { findMatchingCorridor, calculateFeesFromCorridor } = require('@/lib/serviceFeeCalculation');
    ...
    it('should handle no matching corridor', async () => {
      ...
      findMatchingCorridor.mockResolvedValue(null);
    ...
    it('should successfully assign corridor and calculate fees', async () => {
      ...
      findMatchingCorridor.mockResolvedValue(mockCorridorMatch);
    ...
    it('should use pickupCity when pickupLocation.region is null', async () => {
      ...
      expect(result.success).toBe(true);
      expect(findMatchingCorridor).toHaveBeenCalledWith('Addis Ababa', 'Djibouti');
    });
  });
  ...
  describe('business scenarios', () => {
    it('should handle full trip lifecycle: validate → deduct', async () => {
      ...
      const validateResult = await validateWalletBalancesForTrip('load-1', 'carrier-1');
