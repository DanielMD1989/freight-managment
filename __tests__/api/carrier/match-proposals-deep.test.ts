...
// Mock serviceFeeManagement with controllable behavior
...
describe("Match Proposals — Deep Edge Cases", () => {
  ...
  beforeEach(() => {
    ...
    // Default: wallet validation passes
  ...
  describe("Wallet validation", () => {
    it("should return 400 for insufficient carrier balance", async () => {
      mockValidateWallet.mockResolvedValueOnce({
        valid: false,
        shipperFee: 100,
        carrierFee: 50,
        shipperBalance: 10000,
        carrierBalance: 10,
        errors: ["Carrier balance insufficient"],
      });

      const proposal = await db.matchProposal.create({
        data: {
          id: "mp-carrier-balance-deep",
          loadId: seed.load.id,
      ...
      expect(body.error).toContain("Carrier balance insufficient");
    });

    it("should return 400 for insufficient shipper balance", async () => {
      mockValidateWallet.mockResolvedValueOnce({
        valid: false,
        shipperFee: 500,
        carrierFee: 50,
        shipperBalance: 100,
        carrierBalance: 5000,
        errors: ["Shipper balance insufficient"],
      });

      const proposal = await db.matchProposal.create({
        data: {
          id: "mp-shipper-balance-deep",
          loadId: seed.load.id,
          truckId: seed.truck.id,
          proposedById: "dispatcher-user-deep",
          status: "PENDING",
      ...
      expect(res.status).toBe(400);
      const body = await parseResponse(res);
      expect(body.error).toContain("Shipper balance insufficient");
    });

    it("should proceed when balance is sufficient → 200", async () => {
      // Create fresh load and truck to avoid conflicts
      const freshLoad = await db.load.create({
        data: {
          id: "load-wallet-ok-deep",
          status: "POSTED",
          ...
          truckType: "DRY_VAN",
          weight: 5000,
          cargoDescription: "Wallet OK test",
      ...
      const freshTruck = await db.truck.create({
        data: {
          id: "truck-wallet-ok-deep",
          truckType: "DRY_VAN",
          licensePlate: "WAL-OK-1",
          capacity: 10000,
          isAvailable: true,
