...
describe('Phase 2: Foundation Authority Rules', () => {
  ...
  describe('DISPATCHER_COORDINATION_ONLY', () => {
    it('should define dispatcher as coordinator only', () => {
      ...
      expect(RULE_DISPATCHER_COORDINATION_ONLY.id).toBe('DISPATCHER_COORDINATION_ONLY');
    });
    ...
    it('should NOT have ASSIGN_LOADS permission', async () => {
      ...
      const canAssign = await hasPermission('DISPATCHER', Permission.ASSIGN_LOADS);
      expect(canAssign).toBe(false);
    });

    it('should have PROPOSE_MATCH permission', async () => {
      const canProposeMatch = await hasPermission('DISPATCHER', Permission.PROPOSE_MATCH);
  ...
  describe('ONE_ACTIVE_POST_PER_TRUCK', () => {
    it('should define one active post rule', () => {
      ...
      expect(RULE_ONE_ACTIVE_POST_PER_TRUCK.id).toBe('ONE_ACTIVE_POST_PER_TRUCK');
    });
  ...
  describe('Workflow Permissions', () => {
    describe('Match Proposal Workflow', () => {
      it('should allow only DISPATCHER to create proposals', () => {
        expect(canPropose({ role: 'DISPATCHER' as UserRole, organizationId: '', userId: '' })).toBe(true);
        expect(canPropose({ role: 'SHIPPER' as UserRole, organizationId: '', userId: '' })).toBe(false);
        expect(canPropose({ role: 'CARRIER' as UserRole, organizationId: '', userId: '' })).toBe(false);
      });

      it('should allow only CARRIER (owner) to approve proposals', () => {
        const carrierOwner = { role: 'CARRIER' as UserRole, organizationId: 'carrier-1', userId: '' };
        const differentCarrier = { role: 'CARRIER' as UserRole, organizationId: 'carrier-2', userId: '' };
    ...
    describe('Truck Request Workflow', () => {
      it('should allow only SHIPPER (load owner) to create requests', () => {
        const shipperOwner = { role: 'SHIPPER' as UserRole, organizationId: 'shipper-1', userId: '' };
        const differentShipper = { role: 'SHIPPER' as UserRole, organizationId: 'shipper-2', userId: '' };
  ...
  describe('Edge Cases', () => {
    ...
    it('should treat SUPER_ADMIN same as ADMIN for authority', () => {
      const admin = { role: 'ADMIN' as UserRole, organizationId: 'platform', userId: '' };
      const superAdmin = { role: 'SUPER_ADMIN' as UserRole, organizationId: 'platform', userId: '' };
