...
function buildConfig(): Config {
  const nodeEnv = getEnv("NODE_ENV", "development") as
    | "development"
    | "production"
    | "test";
  ...
  return {
    ...
    auth: {
      ...
      jwtEncryptionKey: getEnv(
        "JWT_ENCRYPTION_KEY",
        "dev-encrypt-key-32bytes-padding!"
      ),
    ...
    redis: {
      enabled:
        getEnvBool("REDIS_ENABLED", false) || !!getEnvOrNull("REDIS_URL"),
    ...
    storage: {
      provider: getEnv("STORAGE_PROVIDER", "local") as
        | "local"
        | "s3"
        | "cloudinary",
...
export async function loadSecrets(): Promise<Record<string, string>> {
  ...
  try {
    ...
    console.error(
      "[Config] Failed to load secrets from AWS Secrets Manager:",
      error
    );
    return {};
  }
...
export function validateConfig(cfg?: Config): ConfigValidationError[] {
  ...
  if (c.app.nodeEnv === "production") {
    ...
    if (
      c.auth.jwtSecret === "development-jwt-secret-min-32-chars!" ||
    ...
    if (
      c.auth.jwtEncryptionKey === "dev-encrypt-key-32bytes-padding!" ||
      c.auth.jwtEncryptionKey.includes("dev")
    ) {
      errors.push({
        field: "JWT_ENCRYPTION_KEY",
        message:
          "JWT_ENCRYPTION_KEY must be changed from default in production",
        severity: "error",
      });
    }

    // CRON_SECRET required for scheduled job security
    if (!process.env.CRON_SECRET) {
      errors.push({
        field: "CRON_SECRET",
        message:
          "CRON_SECRET is required in production for cron endpoint security",
        severity: "error",
      });
    }

    // MFA_TOKEN_SECRET should be set (falls back to JWT_SECRET)
    if (!process.env.MFA_TOKEN_SECRET) {
      errors.push({
        field: "MFA_TOKEN_SECRET",
        message:
          "MFA_TOKEN_SECRET not set; falling back to JWT_SECRET. Set a dedicated secret for MFA tokens.",
        severity: "warning",
      });
    }
    ...
    if (!allowedOrigins || allowedOrigins.includes("localhost")) {
      errors.push({
        ...
        message:
          "ALLOWED_ORIGINS not set or contains localhost. Configure production domains.",
        severity: "warning",
      });
    }
    ...
    if (!c.redis.enabled) {
      errors.push({
        field: "REDIS_ENABLED",
        message:
          "Redis is recommended in production for rate limiting and caching",
        severity: "warning",
      });
    }
    ...
    if (c.storage.provider === "local") {
      errors.push({
        ...
        message:
          "Local storage is not recommended in production. Use S3 or Cloudinary.",
        severity: "warning",
      });
    }
...
export function exportAsEnvFormat(): string {
  ...
  const lines: string[] = [
    ...
    c.storage.cdnDomain
      ? `CDN_DOMAIN="${c.storage.cdnDomain}"`
      : `# CDN_DOMAIN not set`,
    ``,
    `# Monitoring`,
