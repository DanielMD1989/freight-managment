...
const portalConfig: Record<
  string,
  { title: string; subtitle: string; backLink: { href: string; label: string } }
> = {
  admin: {
    title: "FreightET",
    subtitle: "Admin",
...
export default function RoleAwareSidebar({
  ...
  const hasAccess = (item: NavItem): boolean => {
    return item.roles.includes(userRole as NavItem["roles"][number]);
  };
  ...
  const isActive = (href: string): boolean => {
    ...
    if (
      hrefPath === "/carrier/loadboard" ||
      hrefPath === "/shipper/loadboard"
    ) {
        ...
        {sections.map((section, sectionIndex) => {
          ...
          return (
            <div key={sectionIndex} className={sectionIndex > 0 ? "pt-4" : ""}>
              {section.title && (
                <div
                  className="mt-2 mb-1 flex items-center gap-2 px-3 py-2"
                  style={{
                    borderTop:
                      sectionIndex > 0
                        ? "1px solid var(--sidebar-border)"
                        : "none",
                    paddingTop: sectionIndex > 0 ? "0.75rem" : "0.5rem",
                  }}
                >
                  <span
                ...
                {visibleItems.map((item) => {
                  ...
                  return (
                    <Link
                      key={item.href}
                      href={item.href}
                      className="group flex items-center gap-3 rounded-lg border-l-[3px] px-3 py-2.5 text-sm font-medium transition-all duration-200"
                      style={{
                        background: active
                          ? "var(--sidebar-active)"
                          : "transparent",
